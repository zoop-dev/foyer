...
export async function onRequestPost({ request, env }) {
  ...
  const row = await consumeCode(env, code);
  ...
  if (redirect_uri && row.redirect_uri !== redirect_uri)
    return json({ error: "redirect_mismatch" }, 400);
  if (!await pkceMatches(code_verifier, row.code_challenge))
    return json({ error: "pkce_failed" }, 400);
  const u = await fetch(
    `${SB_URL(env)}/rest/v1/foyer_users?id=eq.${row.user_id}&select=id,email,name,avatar`,
    { headers: sbH(env) }
  );
