export const SB_URL = (env) =>
  (env.SUPABASE_URL || "https://tvtfoghrdqwssdwvebuo.supabase.co").replace(/\/$/, "");
...
export async function findUser(env, emailNorm) {
  const r = await fetch(
    `${SB_URL(env)}/rest/v1/foyer_users?email_norm=eq.${encodeURIComponent(emailNorm)}&select=*`,
    { headers: sbH(env) }
  );
  return r.ok ? (await r.json())[0] || null : null;
}
export async function createUser(env, { email: email, name: name, passwordHash: passwordHash }) {
...
export async function isLicensedClient(env, domain) {
  ...
  const r = await fetch(
    `${SB_URL(env)}/rest/v1/foyer_sites?domain=eq.${encodeURIComponent(domain)}&select=licensed`,
    { headers: sbH(env) }
  );
...
export async function issueCode(
  env,
  { userId: userId, clientId: clientId, redirectUri: redirectUri, codeChallenge: codeChallenge }
) {
...
export async function consumeCode(env, code) {
  ...
  const r = await fetch(
    `${base}/rest/v1/foyer_auth_codes?code=eq.${encodeURIComponent(code)}&select=*`,
    { headers: sbH(env) }
  );
  const row = r.ok ? (await r.json())[0] : null;
  if (!row) return null;
  await fetch(`${base}/rest/v1/foyer_auth_codes?code=eq.${encodeURIComponent(code)}`, {
    method: "DELETE",
    headers: { ...sbH(env), Prefer: "return=minimal" },
  }).catch(() => {});
...
export async function sessionUser(env, request) {
  ...
  const r = await fetch(
    `${SB_URL(env)}/rest/v1/foyer_auth_sessions?token=eq.${encodeURIComponent(token)}&select=user_id,expires_at`,
    { headers: sbH(env) }
  );
  const s = r.ok ? (await r.json())[0] : null;
  if (!s || new Date(s.expires_at).getTime() < Date.now()) return null;
  const u = await fetch(`${SB_URL(env)}/rest/v1/foyer_users?id=eq.${s.user_id}&select=*`, {
    headers: sbH(env),
  });
...
async function opsSign(env, payload) {
  const key = await crypto.subtle.importKey(
    "raw",
    new TextEncoder().encode(env.FOYER_OPS_PASSWORD || "x"),
...
export function opsClear() {
  return `foyer_ops=; Path=/dashboard; HttpOnly; Secure; SameSite=Lax; Max-Age=0`;
}
...
function qs(params, mode) {
  ...
  ["client_id", "redirect_uri", "state", "code_challenge", "code_challenge_method"].forEach((k) => {
    if (params[k]) u.set(k, params[k]);
  });
  u.set("mode", mode);
  return u.toString();
}
export function consentPage({ clientDomain: clientDomain, user: user, params: params }) {
  const hidden = ["client_id", "redirect_uri", "state", "code_challenge", "code_challenge_method"]
    .map((k) => `<input type="hidden" name="${k}" value="${esc(params[k] || "")}" />`)
    .join("");
