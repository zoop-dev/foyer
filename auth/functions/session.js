...
async function withCors(env, request, res) {
  ...
  let allow = '';
  try { const host = new URL(origin).hostname; if (await isLicensedClient(env, host)) allow = origin; } catch {}
  if (allow) {
...
export async function onRequestGet({ request, env }) {
  ...
  return withCors(env, request, new Response(JSON.stringify(body), { headers: { 'content-type': 'application/json' } }));
}
