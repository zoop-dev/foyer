...
export async function onRequestPost({ request, env }) {
  ...
  switch (a.type) {
    ...
    case 'branding': r = await patch(`foyer_sites?domain=eq.${enc(a.domain)}`, { hide_branding: !!a.value }); break;
    ...
    case 'set_plan': r = await patch(`foyer_sites?domain=eq.${enc(a.domain)}`, { plan: a.value === 'ultra' ? 'ultra' : a.value === 'pro' ? 'pro' : 'free' }); break;
