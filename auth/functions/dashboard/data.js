...
export async function onRequestGet({ request: request, env: env }) {
  ...
  const base = SB_URL(env),
    H = sbH(env);
  const get = (q) =>
    fetch(`${base}/rest/v1/${q}`, { headers: H })
      .then((r) => (r.ok ? r.json() : []))
      .catch(() => []);
  const [sites, heartbeats, announcements, flags, errors, moderation, meta] = await Promise.all([
    get(
      "foyer_sites?select=domain,name,licensed,offline,offline_message,ai_enabled,hide_branding,moderation_config,backup_quota,plan&order=domain"
    ),
    get("foyer_heartbeats?select=domain,live_version,last_seen"),
    get(
      "foyer_announcements?order=created_at.desc&select=id,scope,message,level,active,hide_after,ends_at,created_at"
    ),
    ...
    get(
      "foyer_moderation?order=created_at.desc&limit=100&select=domain,slug,url,reason,block_type,created_at"
    ),
  ...
  const backup_counts = {};
  (bkRows || []).forEach((r) => {
    backup_counts[r.domain] = (backup_counts[r.domain] || 0) + 1;
  });
  const metaMap = {};
  (meta || []).forEach((m) => {
  ...
  try {
    if (metaMap.moderation_config)
      moderation_config = { ...moderation_config, ...JSON.parse(metaMap.moderation_config) };
  } catch (e) {}
  return json({
    sites: sites,
    ...
    latest_version: metaMap.latest_version || "",
  });
}
