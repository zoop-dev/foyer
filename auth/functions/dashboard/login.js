...
export async function onRequestPost({ request, env }) {
  if (!opsIpAllowed(env, request)) return json({ error: 'not_found' }, 404);   // network gate
  const { password } = await request.json().catch(() => ({}));
  if (!env.FOYER_OPS_PASSWORD || !password || !ctEq(password, env.FOYER_OPS_PASSWORD)) return json({ error: 'wrong password' }, 401);
