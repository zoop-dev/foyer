...
const htmlRes = (body, status = 200) =>
  new Response(body, { status: status, headers: { "content-type": "text/html;charset=utf-8" } });
...
function validRedirect(clientId, redirectUri, selfOrigin) {
  try {
    ...
    if (u.hostname === clientId) return true;
    if (selfOrigin && redirectUri === selfOrigin + "/popup-callback") return true;
    return false;
  } catch {
    return false;
  }
...
export async function onRequestGet({ request: request, env: env }) {
  ...
  if (user && !switching)
    return htmlRes(consentPage({ clientDomain: p.client_id, user: user, params: p }));
  const mode = url.searchParams.get("mode") === "signup" ? "signup" : "login";
  return htmlRes(loginPage({ clientDomain: p.client_id, params: p, mode: mode }));
}
export async function onRequestPost({ request: request, env: env }) {
  ...
  if (action === "signup") {
    ...
    if (!user0 || !(await verifyPassword(password, user0.password_hash)))
      return back("login", "Incorrect email or password.");
  }
