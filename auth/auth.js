...
(function () {
  ...
  function pkce() {
    ...
    return crypto.subtle.digest('SHA-256', new TextEncoder().encode(v)).then(function (h) { return { v: v, c: b64url(h) }; });
  }
    ...
    return pkce().then(function (p) {
      return new Promise(function (resolve, reject) {
        var params = new URLSearchParams({
          client_id: location.hostname, redirect_uri: CALLBACK, state: location.origin,
          code_challenge: p.c, code_challenge_method: 'S256', display: 'popup',
        });
        var w = 440, h = 620, l = Math.max(0, (screen.width - w) / 2), t = Math.max(0, (screen.height - h) / 2);
        var popup = window.open(PROVIDER + '/authorize?' + params, 'foyer_auth', 'width=' + w + ',height=' + h + ',left=' + l + ',top=' + t);
        if (!popup) { reject({ error: 'popup_blocked' }); return; }
        var done = false;
        function onMsg(e) {
          if (e.origin !== PROVIDER || !e.data || e.data.type !== 'foyer_auth' || !e.data.code) return;
          done = true; window.removeEventListener('message', onMsg); try { popup.close(); } catch (_) {}
          fetch(exchangeUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ code: e.data.code, code_verifier: p.v, redirect_uri: CALLBACK }) })
            .then(function (r) { return r.json(); }).then(resolve).catch(function () { reject({ error: 'exchange_failed' }); });
        }
        window.addEventListener('message', onMsg);
        var iv = setInterval(function () {
          if (popup.closed && !done) { clearInterval(iv); window.removeEventListener('message', onMsg); reject({ error: 'closed' }); }
        }, 500);
      });
    });
  }
  ...
  function oneTap(opts) {
    ...
    fetch(PROVIDER + '/session', { credentials: 'include' }).then(function (r) { return r.ok ? r.json() : {}; }).then(function (d) {
      ...
      x.onclick = function () { card.style.opacity = '0'; card.style.transform = 'translateY(-12px)'; setTimeout(function () { card.remove(); }, 380); };
      top.appendChild(x);
      ...
      btn.onclick = function () { card.remove(); signIn(opts).then(function (s) { if (opts.onSignedIn) opts.onSignedIn(s); }).catch(function () {}); };
