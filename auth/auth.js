(function () {
  ...
  function oneTap(opts) {
    ...
    fetch(PROVIDER + "/session", { credentials: "include" })
      .then(function (r) {
        return r.ok ? r.json() : {};
      })
      .then(function (d) {
        ...
        x.style.cssText =
          "margin-left:auto;background:none;border:none;color:#8b94a6;font-size:1.1rem;cursor:pointer;line-height:1;";
        x.onclick = function () {
        ...
        who.style.cssText =
          "font-weight:200;font-size:.82rem;line-height:1.5;color:rgba(232,237,242,.82);margin-bottom:.9rem;";
        who.textContent = "Continue as " + (u.name || u.email);
        var btn = document.createElement("button");
        btn.textContent = "Sign in with Foyer";
        btn.style.cssText =
          "width:100%;background:#7fa6d8;color:#070a0e;border:none;font-family:'Josefin Sans',sans-serif;font-weight:300;font-size:.62rem;letter-spacing:.2em;text-transform:uppercase;padding:.7rem;border-radius:8px;cursor:pointer;";
  ...
  function getProfile() {
    return fetch(PROVIDER + "/account", { credentials: "include" })
      .then(function (r) {
