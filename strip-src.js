...
const stripJs = async (code) => {
  ...
  try {
    return (await esbuild.transform(code, { loader: "js", minifyWhitespace: false, legalComments: "none" })).code;
  } catch {
    return code;
  }
};
const SKIP = [/[/\\](deps|node_modules|dist|\.git)[/\\]/, /[/\\]assets[/\\]fonts[/\\]/, /\.(json|md|sql|ico|png|svg|woff2)$/];
