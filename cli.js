...
function run(label, cmd, args, env = process.env, input = null, cwd = ROOT) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const tick = tty
      ...
      : null;
    const p = spawn(cmd, args, { env: env, cwd: cwd });
    if (input != null) {
      p.stdin.write(input);
      p.stdin.end();
    }
...
function passthrough(cmd, args, env = process.env) {
  return new Promise((resolve, reject) => {
    const p = spawn(cmd, args, { stdio: "inherit", env: env, cwd: ROOT });
...
function readLine({ hidden: hidden = false } = {}) {
  return new Promise((resolve) => {
...
async function ask(
  label,
  { def: def = "", required: required = false, hidden: hidden = false } = {}
) {
  for (;;) {
    const hint = hidden
      ? ""
...
async function askColor(label, def) {
  for (;;) {
    const v = await ask(label + " " + swatch(def), { def: def });
    const h = normHex(v);
    if (h) {
...
function previewCard({ bg: bg, accent: accent, text: text, name: name, tagline: tagline }) {
  if (!tty) return;
  const [R, G, B] = _rgb(bg),
    [ar, ag, ab] = _rgb(accent),
    [tr, tg, tb] = _rgb(text);
  const BG = `[48;2;${R};${G};${B}m`,
    RS = "[0m";
  const AC = `[38;2;${ar};${ag};${ab}m`,
    TX = `[38;2;${tr};${tg};${tb}m`;
  const W = 32;
  const row = (fg, s) => {
    s = (s || "").slice(0, W);
...
async function cmdSites() {
  ...
  console.log();
  for (const { name: name, cfg: cfg } of list) {
    const sw = cfg.themeColor || "#888";
    console.log(
      "    " +
        c.br("∩") +
        " " +
        c.bold(name.padEnd(10)) +
...
async function cmdDeploy(target) {
  ...
  console.log(
    "\n  " +
      ok +
      c.green(c.bold(`  deployed ${list.length} site${list.length > 1 ? "s" : ""} · v${v}`)) +
      "\n"
  );
  await sbSetMeta("latest_version", v)
    .then((okv) =>
      console.log(
        "  " +
          (okv ? ok : c.yellow("!")) +
          c.dim(okv ? " global sys version → v" + v : " couldn't sync global version") +
          "\n"
      )
    )
    .catch(() => {});
  await cmdGithub(`deploy v${v}`).catch((e) =>
    console.log("  " + c.yellow("!") + c.dim(" github push skipped: " + e.message))
  );
}
...
async function cmdAuthDeploy() {
  ...
  console.log(
    "    " +
      dot +
      c.dim(" • set the service key: ") +
      c.cyan("npx wrangler pages secret put SUPABASE_SERVICE_KEY --project-name=foyer-auth")
  );
  console.log(
    "    " +
      dot +
      c.dim(" • attach the ") +
      c.cyan("foyer.zo0p.dev") +
      c.dim(" custom domain to the foyer-auth Pages project")
  );
  console.log(
    "    " +
      dot +
      c.dim(" • (optional) put ") +
      c.cyan("FOYER_AUTH_ACCOUNT=<cf-account-id>") +
      c.dim(" in .foyer.env to target a specific account") +
      "\n"
  );
}
async function cmdStage(target) {
  ...
  console.log(
    "\n  " +
      ok +
      c.dim(` staged ${list.length} site${list.length > 1 ? "s" : ""} to preview branch`) +
      "\n"
  );
}
...
function defaultIconSvg({ bg: bg = "#020a03", accent: accent = "#4dbd6a" } = {}) {
  return (
...
async function generateIcons(svg, dir) {
  ...
  console.log(
    "    " +
      ok +
      " generated " +
      c.bold(Object.keys(files).length + 1) +
      c.dim(" icons (svg, ico, png 16→512, apple-touch)")
  );
}
...
async function sbDelete(domain) {
  ...
  if (!key) {
    console.log(
      "    " +
        c.yellow("!") +
        c.dim(" no service key — skipped Supabase removal (delete the row manually)")
    );
    return;
  }
  ...
  console.log(
    "    " +
      ...
      (r.ok
        ...
        : c.dim(" Supabase delete failed (" + r.status + ")"))
  );
}
async function sbRegister({ domain: domain, name: name, project: project }) {
  const key = await sbServiceKey();
  if (!key) {
    console.log(
      "    " +
...
async function cmdNew(initialName) {
  ...
  console.log(
    "  " +
      c.bold("Onboard a new Foyer site") +
      c.dim("  — answer a few questions and I'll create everything.\n")
  );
  ...
  previewCard({ bg: bgColor, accent: themeColor, text: textColor, name: name, tagline: tagline });
  header("cloudflare");
  ...
  for (;;) {
    if (accounts.length) {
      ...
      accounts.forEach((a, i) =>
        console.log(
          "      " +
            c.cyan(String(i + 1).padStart(2)) +
            ". " +
            c.bold(a.name) +
            c.dim("  " + a.id) +
            (knownAccounts.includes(a.id) ? c.dim("  · in use") : "")
        )
      );
      console.log("       " + c.cyan("L") + ". " + c.dim("log in to a different account"));
      const pick = (
        await ask("Pick an account (number), or L to log in", { required: true })
      ).trim();
      if (/^l$/i.test(pick)) {
  ...
  for (;;) {
    iconInput = (
      await ask("Icon SVG (blank = generate an arch mark in your colors)", { def: "" })
    ).trim();
  ...
  if (!isPublic)
    line(
      "methods",
      Object.keys(secrets)
        .filter((k) => /CLIENT_ID|RESEND_API/.test(k))
        .map((k) => k.split("_")[0].toLowerCase())
        .join(", ") + (turnstile ? " + turnstile" : "")
    );
  console.log();
  if (
    !(await confirm(
  ...
  const cfg = {
    ...
    cloudflare: { accountId: accountId, project: project, d1Name: d1Name, d1Id: d1Id },
  };
  const env = acctEnv(cfg);
  header("scaffold");
  ...
  if (!cfg.cloudflare.d1Id) {
    ...
    const id = (d1Out.match(/database_id\s*=\s*"([0-9a-f-]{36})"/i) ||
      d1Out.match(/"(?:uuid|database_id)"\s*:\s*"([0-9a-f-]{36})"/i) ||
      [])[1];
    if (!id)
      die(
  ...
  await sbRegister({ domain: domain, name: name, project: project });
  await cmdDeploy(key);
  header("done");
  console.log(
    "    " +
      ok +
      c.green(c.bold(` ${name} is live`)) +
      c.dim(" once DNS points at the Pages project.")
  );
  console.log(
    "    " +
      dot +
      ...
      c.dim(" at the '") +
      project +
      c.dim("' Pages project (Cloudflare → Pages → Custom domains)")
  );
  if (!isPublic) {
    if (secrets.GOOGLE_CLIENT_ID || secrets.GITHUB_CLIENT_ID || secrets.DISCORD_CLIENT_ID)
      console.log(
        "    " +
          ...
          c.dim(" add ") +
          c.cyan(`https://${domain}`) +
          c.dim(" as an authorized OAuth redirect/origin in each provider's console")
      );
  }
...
async function cmdIcons(name, svgPath) {
  ...
  let svg;
  if (svgPath) {
    svg = svgPath.trim().startsWith("<")
      ? svgPath
      : await readFile(svgPath, "utf8").catch(() => die("can't read " + c.cyan(svgPath)));
...
async function cmdDelete(name) {
  ...
  console.log(
    "      " +
      c.red("•") +
      c.dim(" Cloudflare Pages project ") +
      c.bold(cfg.cloudflare.project) +
      c.dim(" (and its secrets)")
  );
  console.log(
    "      " +
      c.red("•") +
      c.dim(" D1 database ") +
      c.bold(cfg.cloudflare.d1Name) +
      c.dim(" (all its data)")
  );
  console.log(
  ...
  await passthrough("npx", ["wrangler", "d1", "delete", cfg.cloudflare.d1Name], env).catch((e) =>
    console.log("      " + c.yellow("! ") + c.dim("D1: " + e.message))
  );
  ...
  console.log(
    "\n  " +
      ok +
      c.green(c.bold(`  '${name}' deleted`)) +
      c.dim(" — remember to remove the custom domain in Cloudflare if it was attached.") +
      "\n"
  );
}
async function cmdSecret(name, key) {
  ...
  console.log(
    "    " +
      dot +
      c.dim(" redeploy for it to take effect: ") +
      c.cyan(`foyer deploy ${name}`) +
      "\n"
  );
}
...
async function cmdSetRole(name, email, role) {
  ...
  console.log(
    "    " +
      ...
      c.dim(" → ") +
      c.bold(dbRole || "none") +
      c.dim(`  (${count} account${count > 1 ? "s" : ""} · live immediately, no redeploy)`)
  );
  console.log();
}
  ...
  if (next) {
    ...
    console.log(
      "\n  " +
        ...
        " version → " +
        c.bold("v" + n) +
        c.dim("  (deploy to ship it — the global sys version updates on deploy)") +
        "\n"
    );
  } else {
    ...
    console.log(
      "\n  " +
        ...
        (global
          ? c.dim("   · global latest ") +
            (global === local ? c.green("v" + global) : c.yellow("v" + global))
          : "") +
        "\n"
    );
  }
  ...
  for (const { name: name, cfg: cfg } of list) {
    ...
    console.log(
      "    " +
        ...
        c.bold(name.padEnd(10)) +
        c.dim("live v") +
        (okv ? c.green(live) : c.yellow(live)) +
        c.dim("  / local v" + local) +
...
async function cmdOffline(name, on, reason) {
  ...
  if (!key)
    die(
      "needs the service key. Put " +
        c.cyan("FOYER_SB_SERVICE_KEY=<key>") +
        " in " +
        c.cyan(".foyer.env") +
        " (gitignored)."
    );
  ...
  console.log(
    "    " +
      ok +
      ` ${domain} is now ` +
      (on ? c.yellow("OFFLINE") : c.green("ONLINE")) +
      c.dim("  (edge cache clears within ~60s)")
  );
...
async function cmdLicense(name, on, reason) {
  ...
  console.log(
    "    " +
      ok +
      ` ${domain} is now ` +
      (on ? c.green("LICENSED") : c.red("UNLICENSED")) +
      c.dim("  (edge cache clears within ~60s)")
  );
  if (!on && reason) console.log("    " + dot + c.dim(' message: "') + reason + c.dim('"'));
  console.log();
...
async function cmdBypass(name, kind, code) {
  ...
  if (clear) {
    ...
    console.log(
      "    " +
        dot +
        c.dim(" view:  visit the offline screen → “Operator access”, or ") +
        c.cyan(`https://${domain}/?__fb=${encodeURIComponent(code)}`)
    );
...
async function cmdRegistry() {
  ...
  for (const s of await sr.json()) {
    ...
    const ver = h?.live_version
      ? gv && h.live_version !== gv
        ? c.yellow("v" + h.live_version + " behind")
        : c.green("v" + h.live_version)
      : c.dim("—");
    console.log(
      "    " +
        dot +
        " " +
        c.bold(s.domain.padEnd(18)) +
        (s.licensed ? c.green("licensed") : c.red("unlicensed")) +
        (s.offline ? c.yellow(" · offline") : "") +
        "  " +
        ver +
        c.dim("  · " + timeAgo(h?.last_seen))
    );
  }
  console.log();
}
async function cmdAnnounce(scope, ...argv) {
  ...
  if (sub === "list" || sub === "ls") {
    ...
    for (const a of rows)
      console.log(
        "    " +
          (a.active ? c.green("●") : c.grey("○")) +
          " " +
          (a.level === "warn" ? c.yellow("[warn]") : c.dim("[info]")) +
          " " +
          a.message +
          (a.ends_at ? c.dim("  ends " + a.ends_at) : "")
      );
    console.log();
    return;
  }
  if (sub === "clear" || sub === "off") {
    ...
    console.log(
      "    " +
        (r.ok ? ok : bad) +
        (r.ok ? " deactivated active announcements (kept in history)" : " failed") +
        "\n"
    );
    return;
  }
  if (sub === "remove" || sub === "rm" || sub === "delete") {
    ...
    console.log(
      "    " +
        (r.ok ? ok : bad) +
        (r.ok
          ? " permanently removed all announcements for " +
            (target === "global" ? "all sites" : target)
          : " failed") +
        "\n"
    );
    return;
  ...
  console.log(
    "    " +
      ok +
      " banner live on " +
      c.bold(target === "global" ? "all sites" : target) +
      (flags.level === "warn" ? c.yellow("  [warn]") : "") +
      (flags.hide_after ? c.dim(`  · auto-hides after ${flags.hide_after}s`) : "") +
      c.dim("  (clients pick it up within ~a load)")
  );
  console.log();
}
async function cmdFlag(scope, fkey, value) {
  ...
  console.log(
    "    " +
      ...
      c.dim(" = ") +
      c.cyan(String(val)) +
      c.dim(" on " + (target === "global" ? "all sites" : target)) +
      "\n"
  );
}
  ...
  for (const e of rows)
    console.log(
      "    " +
        c.red("•") +
        " " +
        c.dim(timeAgo(e.created_at).padStart(7)) +
        "  " +
        c.grey((e.domain || "").padEnd(14)) +
        e.message +
        c.dim("  " + (e.url || ""))
    );
  console.log();
...
async function cmdChangelog(...argv) {
  ...
  const rec = { version: String(version), tag: tag, title: title, body: flags.body || "" };
  ...
  console.log(
    "    " +
      ...
      "  " +
      c.bold(title) +
      (rec.published === false ? c.dim("  (draft)") : "")
  );
  if (rec.body)
    console.log(
      "    " +
        dot +
        c.dim(" " + rec.body.replace(/\n/g, " ").slice(0, 70) + (rec.body.length > 70 ? "…" : ""))
    );
  console.log();
}
function help() {
  banner();
  const row = (cmd, desc) =>
    "    " + c.cyan(cmd) + " ".repeat(Math.max(2, 30 - cmd.length)) + c.dim(desc);
  ...
  console.log(
    "\n  " +
      c.dim("examples:") +
      c.cyan("  foyer deploy all") +
      c.dim("   ·  ") +
      c.cyan("foyer status all") +
      "\n"
  );
}
...
const table = {
  ...
  deploy: () =>
    args[0] === "github" || args[0] === "gitea"
      ? cmdGithub(args.slice(1).join(" "))
      : cmdDeploy(args[0]),
