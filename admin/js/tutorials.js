...
let _tutPublished = null;   // published form-state of the open tutorial (for diffing)
function tutDraftKey(id) { return 'foyer_tutdraft_' + id; }
function tutFormState() {
  const g = id => document.getElementById(id);
  if (!g('tutContent')) return null;
  return { title:g('tutTitle').value, slug:g('tutSlug').value, description:g('tutDesc').value, cover_image:g('tutCover').value, content:g('tutContent').value };
}
function tutSaveDraft() {
  if (_tutSel == null) return;
  const f = tutFormState(); if (!f) return;

  if (_tutPublished && JSON.stringify(f) === JSON.stringify(_tutPublished)) {
    tutClearDraft(_tutSel);
    const st = document.getElementById('tutStatus'); if (st) st.textContent = '';
    renderTutList();
    return;
  }
  try { _ls.setItem(tutDraftKey(_tutSel), JSON.stringify({ state:f, ts:Date.now() })); } catch {}
  const st = document.getElementById('tutStatus'); if (st) st.textContent = 'Saved draft';
  renderTutList();
}
function tutClearDraft(id) { try { _ls.removeItem(tutDraftKey(id)); } catch {} }
function tutUnsaved(t) { try { return !!_ls.getItem(tutDraftKey(t.id)); } catch { return false; } }
setInterval(() => { if (document.getElementById('sec-tutorials')?.classList.contains('active')) tutSaveDraft(); }, 4000);
...
function renderTutList() {
  ...
  el.querySelectorAll('[data-tid]').forEach(row => {
    row.addEventListener('click', e => { if (e.target.closest('[data-discard]')) return; selectTutorial(+row.dataset.tid); });
  });
  el.querySelectorAll('[data-discard]').forEach(badge => badge.addEventListener('click', async e => {
    e.stopPropagation();
    const id = +badge.dataset.discard;
    if (!await dlg.confirm('Discard unsaved changes for this tutorial?', { confirm: 'Discard', danger: true })) return;
    tutClearDraft(id);
    renderTutList();
    if (id === _tutSel) {
      _tutDirty = false;
...
async function selectTutorial(id) {
  if (_tutDirty && !await dlg.confirm('You have unsaved changes. Discard them?', { confirm: 'Discard', danger: true })) return;
  _tutDirty = false;
  _tutSel = id;
  renderTutList();
  const res = await fetch(`/api/tutorials/${id}`, { headers: authHeaders() });
  ...
  _tutPublished = { title:t.title, slug:t.slug, description:t.description, cover_image:t.cover_image, content:t.content||'' };
...
function renderTutEditor(t) {
  ...
  ['tutTitle','tutSlug','tutDesc','tutCover','tutContent'].forEach(id => {
    document.getElementById(id)?.addEventListener('input', () => { _tutDirty = true; document.getElementById('tutStatus').textContent = ''; tutSaveDraft(); });
  });

  document.getElementById('tutPickImg').addEventListener('click', () => {
    openImgPicker(url => { document.getElementById('tutCover').value = url; _tutDirty = true; tutSaveDraft(); });
  });

  wireMdToolbar(el, document.getElementById('tutContent'), () => { _tutDirty = true; tutSaveDraft(); });
...
async function createTutorial() {
  ...
  const title = await new Promise(resolve => {
    ...
    inp.focus();
    bd.querySelector('#_tutCancel').addEventListener('click', () => { bd.remove(); resolve(null); });
    bd.querySelector('#_tutCreate').addEventListener('click', () => { bd.remove(); resolve(inp.value); });
    inp.addEventListener('keydown', e => { if (e.key === 'Enter') { bd.remove(); resolve(inp.value); } if (e.key === 'Escape') { bd.remove(); resolve(null); } });
  });
  ...
  const res = await fetch('/api/tutorials', {
    method: 'POST',
    headers: { ...authHeaders(), 'Content-Type': 'application/json' },
    body: JSON.stringify({ title: title.trim(), slug, description: '', content: '', cover_image: '' }),
  });
  if (!res.ok) { await dlg.alert('Failed to create tutorial.'); return; }
  const { id } = await res.json();
  _tutList.unshift({ id, title: title.trim(), slug, description: '', cover_image: '', created_at: new Date().toISOString() });
  _tutDirty = false;
  _tutSel = id;
  renderTutList();
  renderTutEditor({ id, title: title.trim(), slug, description: '', content: '', cover_image: '' });
}
