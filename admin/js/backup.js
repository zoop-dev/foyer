import { authHeaders, dlg, toast } from "./utils.js";
...
async function bkDownload() {
  ...
  try {
    ...
    const filename = `${String(name).toLowerCase().replace(
      /[^a-z0-9]+/g,
      "-"
    )}-${scope}-${/* @__PURE__ */ (/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.foyer`;
...
function bkFileChosen(file) {
  ...
  reader.onload = () => {
    ...
    try {
      ...
      bkEl("bkSummary").innerHTML = '<span style="color:#e0556a;">That file isn’t a valid Foyer backup.</span>';
      return;
    }
    if (b && b.foyer_enc === 1) {
...
export async function renderBackupTab() {
  if (!_bkWired) {
    _bkWired = true;
  ...
  try {
    ...
    if (Array.isArray(pages))
      bkEl("bkPage").innerHTML = pages.map(
        (p) => `<option value="${(p.slug || "").replace(/"/g, "&quot;")}">${(p.title || p.slug || "").replace(/</g, "&lt;")} — ${(p.slug || "").replace(/</g, "&lt;")}</option>`
      ).join("");
  } catch (e) {
  }
}
