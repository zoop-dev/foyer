...
let _revSel = null;
let _revDirty = false;
function revDraftKey(id) {
  return "foyer_revdraft_" + id;
}
function revFormState() {
  const g = (id) => document.getElementById(id);
  if (!g("revContent")) return null;
  return {
    title: g("revTitle").value,
    slug: g("revSlug").value,
    description: g("revDesc").value,
    cover_image: g("revCover").value,
    rating: parseInt(g("revRating").value) || 0,
    content: g("revContent").value,
  };
}
let _revPublished = null;
function revSaveDraft() {
  if (_revSel == null) return;
  const f = revFormState();
  if (!f) return;
  ...
  try {
    _ls.setItem(revDraftKey(_revSel), JSON.stringify({ state: f, ts: Date.now() }));
  } catch {}
  const st = document.getElementById("revStatus");
  if (st) st.textContent = "Saved draft";
  renderRevList();
}
function revClearDraft(id) {
  try {
    _ls.removeItem(revDraftKey(id));
  } catch {}
}
function revUnsaved(t) {
  try {
    return !!_ls.getItem(revDraftKey(t.id));
  } catch {
    return false;
  }
}
setInterval(() => {
  if (document.getElementById("sec-reviews")?.classList.contains("active")) revSaveDraft();
}, 4e3);
...
function renderRevList() {
  ...
  if (!_revList.length) {
    el.innerHTML =
      '<p style="font-size:.62rem;font-weight:100;color:var(--muted);padding:1rem .5rem;">No reviews yet.</p>';
    return;
  }
  el.innerHTML = _revList
    .map(
      (t) =>
  ...
  el.querySelectorAll("[data-discard]").forEach((badge) =>
    badge.addEventListener("click", async (e) => {
      ...
      if (
        !(await dlg.confirm("Discard unsaved changes for this review?", {
          confirm: "Discard",
          danger: true,
        }))
      )
    ...
    if (raw) {
      ...
      const differs =
        ...
        (d.state.title !== t.title ||
          ...
          (parseInt(d.state.rating) || 0) !== (parseInt(t.rating) || 0) ||
...
function renderRevEditor(t) {
  ...
  if (!el) return;
  const ratingOpts = [0, 1, 2, 3, 4, 5]
    .map(
      (n) =>
        `<option value="${n}"${(parseInt(t.rating) || 0) === n ? " selected" : ""}>${n === 0 ? "No rating" : "★".repeat(n)}</option>`
    )
    .join("");
  ...
  document.getElementById("revPickImg").addEventListener("click", () => {
    openImgPicker((url) => {
      document.getElementById("revCover").value = url;
      _revDirty = true;
      revSaveDraft();
    });
  });
  wireMdToolbar(el, document.getElementById("revContent"), () => {
    _revDirty = true;
    revSaveDraft();
  });
  ...
  document.getElementById("revDelBtn").addEventListener("click", async () => {
    ...
    _revDirty = false;
    renderRevList();
    document.getElementById("revEditor").innerHTML =
      '<p style="font-weight:100;font-size:.68rem;color:var(--muted);margin:auto;text-align:center;">Select a review or create a new one.</p>';
    toast("Deleted");
  });
}
async function createReview() {
  ...
  const result = await new Promise((resolve) => {
    ...
    const done = (ok) => {
      bd.remove();
      resolve(ok ? { title: titleInp.value, slug: slugInp.value } : null);
    };
  ...
  if (res.status === 409) {
    await dlg.alert("That slug is already taken.");
    return;
  }
  if (!res.ok) {
    await dlg.alert("Failed to create review.");
    return;
  }
