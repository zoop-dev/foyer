...
function revSaveDraft() {
  ...
  if (_revPublished && JSON.stringify(f) === JSON.stringify(_revPublished)) {
    ...
    const st2 = document.getElementById("revStatus");
...
function renderRevList() {
  ...
  el.querySelectorAll("[data-discard]").forEach(
    (badge) => badge.addEventListener("click", async (e) => {
      ...
      if (!await dlg.confirm("Discard unsaved changes for this review?", {
        confirm: "Discard",
        danger: true
      }))
        return;
...
async function selectReview(id) {
  if (_revDirty && !await dlg.confirm("You have unsaved changes. Discard them?", {
    confirm: "Discard",
    danger: true
  }))
    return;
...
function renderRevEditor(t) {
  ...
  if (!el) return;
  const ratingOpts = [0, 1, 2, 3, 4, 5].map(
    (n) => `<option value="${n}"${(parseInt(t.rating) || 0) === n ? " selected" : ""}>${n === 0 ? "No rating" : "★".repeat(n)}</option>`
  ).join("");
  el.innerHTML = `
...
async function createReview() {
  ...
  const result = await new Promise((resolve) => {
    ...
    titleInp.addEventListener("input", () => {
      if (!slugEdited)
        slugInp.value = titleInp.value.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
    });
    const done = (ok) => {
      bd.remove();
