...
function getHiddenTabs() {
  try {
    return JSON.parse(localStorage.getItem("foyer_hidden_tabs") || "[]");
  } catch {
    return [];
  }
}
function setHiddenTabs(arr) {
  try {
    localStorage.setItem("foyer_hidden_tabs", JSON.stringify(arr));
  } catch {}
}
function applyHiddenTabs() {
  const hidden = getHiddenTabs();
  document.querySelectorAll(".tab-btn").forEach((b) => {
    if (HIDEABLE_TABS.includes(b.dataset.tab))
      b.style.display = hidden.includes(b.dataset.tab) ? "none" : "";
  });
...
function renderTabToggles() {
  ...
  if (!el) return;
  const hidden = getHiddenTabs();
  el.innerHTML = HIDEABLE_TABS.map(
    (t) =>
      `<label style="display:flex;align-items:center;gap:.6rem;cursor:pointer;font-size:.72rem;font-weight:200;color:rgba(220,245,225,.8);padding:.25rem 0;"><input type="checkbox" data-tabtoggle="${t}" ${hidden.includes(t) ? "" : "checked"} /> ${TAB_LABELS[t]}</label>`
  ).join("");
  el.querySelectorAll("input[data-tabtoggle]").forEach((cb) => {
    cb.addEventListener("change", () => {
      let h = getHiddenTabs();
      const t = cb.dataset.tabtoggle;
      if (cb.checked) h = h.filter((x) => x !== t);
      else if (!h.includes(t)) h.push(t);
      setHiddenTabs(h);
      applyHiddenTabs();
    });
  });
...
async function fetchSettings() {
  ...
  document.getElementById("sSearchEnabled").checked = s.search_enabled !== "0";
  document.getElementById("sSmoothScroll").checked = s.smooth_scroll !== "0";
  ...
  document.getElementById("sSignupDomainLimit").value =
    s.signup_domain_limit && s.signup_domain_limit !== "0" ? s.signup_domain_limit : "";
  ...
  document.getElementById("sSignupDomainExempt").value =
    s.signup_domain_exempt != null ? s.signup_domain_exempt : DEF_EXEMPT;
}
document.getElementById("saveSettingsBtn").addEventListener("click", async () => {
  const sp = document.getElementById("settingsSpinner"),
    ss = document.getElementById("settingsStatus"),
    btn = document.getElementById("saveSettingsBtn");
  ...
  const res = await fetch("/api/settings", {
    ...
    body: JSON.stringify({
      ...
      gate_btn: document.getElementById("sGateBtn").value.trim(),
      gate_bg: document.getElementById("sGateBg").value.trim(),
      ...
      signup_domain_limit: String(
        parseInt(document.getElementById("sSignupDomainLimit").value, 10) || 0
      ),
      signup_domain_window_h: String(
        parseInt(document.getElementById("sSignupDomainWindow").value, 10) || 24
      ),
  ...
  btn.disabled = false;
  ss.textContent = res.ok ? "Saved." : "Error.";
});
let _i18nCodes = null;
async function buildLangPicker(selected) {
  const base = document.getElementById("sLangBase"),
    opts = document.getElementById("sLangOptions");
  if (!base || !opts) return;
  if (!_i18nCodes) {
    try {
      _i18nCodes = Object.keys(await fetch("/assets/i18n.json").then((r) => r.json()));
    } catch {
      _i18nCodes = ["en"];
    }
  }
  ...
  base.innerHTML = _i18nCodes
    .map(
      (c) => `<option value="${c}"${c === baseSel ? " selected" : ""}>${name(c)} (${c})</option>`
    )
    .join("");
  opts.innerHTML = _i18nCodes
    .filter((c) => c !== baseSel)
    .map(
      (c) =>
        `<label style="display:flex;align-items:center;gap:.35rem;cursor:pointer;font-size:.7rem;font-weight:200;color:rgba(220,245,225,.8);"><input type="checkbox" data-lang="${c}" ${extras.has(c) ? "checked" : ""} style="width:auto;" /> ${name(c)} <span style="opacity:.45;">${c}</span></label>`
    )
    .join("");
  base.onchange = () =>
    buildLangPicker([
      base.value,
      ...[...opts.querySelectorAll("input[data-lang]:checked")]
        .map((i) => i.dataset.lang)
        .filter((c) => c !== base.value),
    ]);
}
function getLanguagesValue() {
  const base = document.getElementById("sLangBase")?.value || "en";
  const extras = [...document.querySelectorAll("#sLangOptions input[data-lang]:checked")].map(
    (i) => i.dataset.lang
  );
  return extras.length ? [base, ...extras].join(",") : "";
}
let _navCustomLinks = [];
let _navPageOrder = null;
async function loadNavEditor() {
  ...
  if (sRes.ok) {
    ...
    try {
      _navCustomLinks = JSON.parse(s.nav_custom_links || "[]");
    } catch {
      _navCustomLinks = [];
    }
  }
...
function renderNavPageList(pages) {
  ...
  if (!pages.length) {
    el.innerHTML =
      '<p style="font-size:.62rem;color:var(--muted);font-weight:100;">No published pages yet.</p>';
    return;
  }
  el.innerHTML = pages
    .map(
      (p, i) =>
        `\n    <div class="nav-page-row" data-pid="${p.id}" data-idx="${i}" draggable="true" style="display:flex;align-items:center;gap:.6rem;padding:.45rem .6rem;border:1px solid var(--border);background:var(--surface);cursor:grab;">\n      <span style="font-size:.9rem;color:var(--muted);cursor:grab;">⋮⋮</span>\n      <span style="flex:1;font-weight:200;font-size:.72rem;color:rgba(220,245,225,.8);">${escHtml(p.title)}</span>\n      <span style="font-size:.6rem;color:var(--muted);font-family:monospace;">${escHtml(p.slug)}</span>\n      <select data-parent-pid="${p.id}" title="Sub-page of…" style="font-size:.58rem;font-weight:200;background:var(--bg);color:rgba(220,245,225,.8);border:1px solid var(--border);padding:.25rem .35rem;max-width:130px;">\n        <option value="">— top level —</option>\n        ${pages
          ...
          .map(
            (o) =>
              `<option value="${escAttr(o.slug)}" ${p.parent === o.slug ? "selected" : ""}>↳ ${escHtml(o.title)}</option>`
          )
          .join(
            ""
  ...
  el.querySelectorAll(".nav-page-row").forEach((row) => {
    ...
    row.querySelector("input[type=checkbox]").addEventListener("change", async (e) => {
      ...
      let state = {};
      try {
        state = JSON.parse(page.page_json || "{}");
      } catch {}
      ...
      await fetch(`/api/pages/${pid}`, {
        ...
        body: JSON.stringify({
          title: page.title,
          slug: page.slug,
          page_json: JSON.stringify(state),
    ...
    row.querySelector("select[data-parent-pid]").addEventListener("change", async (e) => {
      ...
      if (e.target.value) state.parent = e.target.value;
...
function renderNavCustomLinks() {
  ...
  if (!_navCustomLinks.length) {
    el.innerHTML =
      '<p style="font-size:.62rem;font-weight:100;color:var(--muted);">No custom links yet.</p>';
  } else {
    el.innerHTML = _navCustomLinks
      .map(
        (l, i) =>
    ...
    el.querySelectorAll("[data-cl-rm]").forEach((btn) => {
      btn.addEventListener("click", () => {
        _navCustomLinks.splice(+btn.dataset.clRm, 1);
        renderNavCustomLinks();
        saveNavCustomLinks();
      });
    });
...
function cropImage(file) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      ov.style.cssText =
        "position:fixed;inset:0;z-index:9000;background:rgba(6,12,8,.92);backdrop-filter:blur(6px);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1rem;padding:1.5rem;";
      ov.innerHTML = `\n        <div style="font-weight:200;font-size:.72rem;letter-spacing:.16em;text-transform:uppercase;color:var(--accent);">Crop · ${escHtml(file.name)}</div>\n        <div class="crop-stage" style="position:relative;line-height:0;max-width:90vw;max-height:62vh;box-shadow:0 20px 60px rgba(0,0,0,.6);touch-action:none;user-select:none;">\n          <img class="crop-img" draggable="false" style="display:block;max-width:90vw;max-height:62vh;width:auto;height:auto;" />\n          <div class="crop-box" style="position:absolute;border:1px solid rgba(255,255,255,.9);box-shadow:0 0 0 9999px rgba(6,12,8,.6);cursor:move;">\n            ${["nw", "ne", "sw", "se", "n", "s", "e", "w"].map((h) => `<div class="crop-h" data-h="${h}" style="position:absolute;width:14px;height:14px;background:var(--accent);border:1px solid rgba(0,0,0,.4);"></div>`).join("")}\n          </div>\n        </div>\n        <div style="display:flex;align-items:center;gap:.4rem;flex-wrap:wrap;justify-content:center;">\n          ${[
        ...
        .map(
          ([v, l], i) =>
            `<button class="crop-ar${i === 0 ? " on" : ""}" data-ar="${v}" style="font-family:inherit;font-size:.58rem;letter-spacing:.1em;text-transform:uppercase;font-weight:200;padding:.35rem .7rem;border:1px solid var(--border);background:${i === 0 ? "rgba(var(--accent-rgb),.18)" : "transparent"};color:var(--white);cursor:pointer;">${l}</button>`
        )
        .join(
          ""
      ...
      function place(h, x, y) {
        ...
        box.querySelector(`[data-h="${h}"]`).style.cssText +=
          `;${{ nw: `left:${m}px;top:${m}px;cursor:nwse-resize`, ne: `right:${m}px;top:${m}px;cursor:nesw-resize`, sw: `left:${m}px;bottom:${m}px;cursor:nesw-resize`, se: `right:${m}px;bottom:${m}px;cursor:nwse-resize`, n: `left:${mid};top:${m}px;cursor:ns-resize`, s: `left:${mid};bottom:${m}px;cursor:ns-resize`, e: `right:${m}px;top:${mid};cursor:ew-resize`, w: `left:${m}px;top:${mid};cursor:ew-resize` }[h]}`;
      }
      ...
      function down(e, mode) {
        ...
        drag = { mode: mode, sx: p.clientX, sy: p.clientY, cx: cx, cy: cy, cw: cw, ch: ch };
      }
      function move(e) {
        if (!drag) return;
        const p = "touches" in e ? e.touches[0] : e;
        let dx = p.clientX - drag.sx,
        ...
        L = Math.max(0, Math.min(L, R - 20));
        T = Math.max(0, Math.min(T, B - 20));
        R = Math.min(dispW, Math.max(R, L + 20));
        B = Math.min(dispH, Math.max(B, T + 20));
        cx = L;
        cy = T;
        cw = R - L;
        ch = B - T;
      ...
      ov.querySelectorAll(".crop-ar").forEach((b) => {
        b.addEventListener("click", () => {
          ov.querySelectorAll(".crop-ar").forEach((o) => {
      ...
      ov.querySelector(".crop-cancel").addEventListener("click", () => {
        teardown();
        resolve(null);
      });
      ov.querySelector(".crop-skip").addEventListener("click", () => {
        teardown();
        resolve(file);
      });
  ...
  return new Promise((resolve) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        resolve({ data: data, size: size, mime: "image/jpeg" });
      };
...
function renderImgTabGallery() {
  ...
  if (!_imgList.length) {
    gallery.innerHTML =
      '<p style="font-weight:200;font-size:.68rem;color:var(--muted);">No images yet — upload one above.</p>';
    return;
  }
  gallery.innerHTML = _imgList
    .map(
      (img) =>
...
async function uploadFiles(files, nameOverride) {
  for (const original of files) {
    const file = await cropImage(original);
    if (!file) {
      toast(`Skipped ${original.name}`);
      continue;
    }
    const { data: data, size: size, mime: mime } = await compressImage(file);
    if (size > 9e5) {
      toast(`${file.name} too large after compression (${fmtBytes(size)})`, true);
      continue;
    }
    ...
    const res = await fetch("/api/images", {
      method: "POST",
      headers: { ...authHeaders(), "Content-Type": "application/json" },
      body: JSON.stringify({ name: name, data: data, mime: mime, size: size }),
    });
    if (res.ok) {
      const { id: id } = await res.json();
      _imgList.unshift({ id: id, name: name, size: size, created_at: new Date().toISOString() });
      toast(`Uploaded: ${name}`);
    } else {
...
async function openCamera() {
  if (!navigator.mediaDevices?.getUserMedia) {
    toast("Camera not supported on this device.", true);
    return;
  }
  let stream = null,
    facing = "environment",
  ...
  const video = ov.querySelector(".cam-video"),
    shot = ov.querySelector(".cam-shot");
  const showLive = (live) => {
  ...
  async function start() {
    ...
    try {
      ...
      toast("Couldn’t open the camera — check permissions.", true);
      close();
    }
  }
  ov.querySelector("#camX").onclick = close;
  ov.querySelector("#camFlip").onclick = () => {
  ...
  ov.querySelector("#camRetake").onclick = () => {
    shotBlob = null;
    showLive(true);
  };
...
async function recropImage(id) {
  let resp = null;
  try {
    resp = await fetch(`/api/images/${id}?t=${Date.now()}`, { cache: "reload" });
  } catch {}
  if (!resp || !resp.ok) {
    toast("Couldn’t load that image.", true);
    return;
  }
  ...
  if (!cropped) return;
  const { data: data, size: size, mime: mime } = await compressImage(cropped);
  if (size > 9e5) {
    toast(`Too large after crop (${fmtBytes(size)})`, true);
    return;
  }
  const r = await fetch(`/api/images/${id}`, {
    method: "PUT",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify({ data: data, mime: mime, size: size }),
  });
  if (!r.ok) {
    toast("Crop failed to save.", true);
    return;
  }
  bustImgCache(id);
  toast("Image updated ✓");
  const thumb = document.querySelector(`.img-card[data-img-id="${id}"] img`);
  if (thumb) thumb.src = `/api/images/${id}?t=${Date.now()}`;
}
let _fileList = [];
...
function renderFileList() {
  ...
  if (!_fileList.length) {
    el.innerHTML =
      '<p style="font-weight:200;font-size:.68rem;color:var(--muted);">No files yet — upload one above.</p>';
    return;
  }
  el.innerHTML = _fileList
    .map(
      (f) =>
...
async function uploadFileItems(files) {
  for (const file of files) {
    ...
    if (file.size > MAX) {
      toast(`${file.name} is too large (max ~1.5 MB)`, true);
      continue;
    }
...
function openImgPicker(callback) {
  ...
  grid.innerHTML = _imgList.length
    ? _imgList
        .map(
          (img) =>
            `<div class="img-picker-item" data-id="${img.id}" title="${escHtml(img.name || "")}"><img src="/api/images/${img.id}" alt="" /><div class="img-picker-item-name">${escHtml(img.name || "untitled")}</div><div style="font-size:.5rem;color:rgba(77,189,106,.35);padding:0 .4rem .35rem;font-family:monospace;">/api/images/${img.id}</div></div>`
        )
        .join("")
...
function openFilePicker(callback) {
  ...
  fetch("/api/files", { headers: authHeaders() })
    ...
    .then((files) => {
      if (!files.length) {
        list.innerHTML =
          '<p style="font-weight:200;font-size:.68rem;color:var(--muted);padding:1rem;">No files yet — go to the Files tab to upload.</p>';
        return;
      }
      list.innerHTML = files
        .map(
          (f) =>
            `<div class="file-pick-row" data-furl="/api/files/${f.id}"><span style="font-size:1.1rem;">${fileIcon(f.mime)}</span><span class="file-pick-row-name">${escHtml(f.name)}</span><span class="file-pick-row-meta">${fmtBytes(f.size)}</span></div>`
        )
        .join("");
      list.querySelectorAll("[data-furl]").forEach((row) => {
        row.addEventListener("click", () => {
          if (_filePickerCallback) _filePickerCallback(row.dataset.furl);
          overlay.classList.remove("open");
        });
      });
    })
    .catch(() => {
      list.innerHTML =
        '<p style="color:var(--muted);padding:1rem;font-size:.7rem;">Error loading files.</p>';
    });
  overlay.classList.add("open");
}
...
async function fetchBlocklist() {
  ...
  if (!list.length) {
    el.innerHTML =
      '<p style="font-size:.65rem;font-weight:100;color:var(--muted);">No emails blocked yet.</p>';
    return;
  }
  el.innerHTML = list
    .map(
  ...
  el.querySelectorAll(".bl-remove-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      if (
        !(await dlg.confirm(
          `Remove ${btn.dataset.email} from blocklist? This will also restore any linked accounts.`,
          { confirm: "Remove", danger: false }
        ))
      )
        return;
      await fetch(`/api/banned-emails/${encodeURIComponent(btn.dataset.email)}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      fetchBlocklist();
...
document.getElementById("blocklistAddBtn").addEventListener("click", async () => {
  ...
  if (!res.ok) {
    await dlg.alert("Failed to add email to blocklist.");
    return;
  }
  input.value = "";
  toast(`Blocked: ${email}`);
...
async function fetchAllowlist() {
  ...
  if (!list.length) {
    el.innerHTML =
      '<p style="font-size:.65rem;font-weight:100;color:var(--muted);">No emails allowlisted yet.</p>';
    return;
  }
  el.innerHTML = list
    .map(
  ...
  el.querySelectorAll(".al-remove-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      if (
        !(await dlg.confirm(`Remove ${btn.dataset.email} from the allowlist?`, {
          confirm: "Remove",
        }))
      )
        return;
      await fetch(`/api/allowed-emails/${encodeURIComponent(btn.dataset.email)}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      fetchAllowlist();
...
document.getElementById("allowlistAddBtn").addEventListener("click", async () => {
  ...
  if (!res.ok) {
    await dlg.alert("Failed to add email to allowlist.");
    return;
  }
  input.value = "";
  toast(`Allowed: ${email}`);
...
document.querySelectorAll(".set-cat").forEach((b) =>
  b.addEventListener("click", () => {
    const cat = b.dataset.cat;
    document.querySelectorAll(".set-cat").forEach((x) => x.classList.toggle("active", x === b));
    document
      .querySelectorAll(".set-panel")
      .forEach((p) => p.classList.toggle("active", p.dataset.panel === cat));
  })
);
let _hxPages = null;
async function renderHistoryTab() {
  const sel = document.getElementById("hxPage"),
    list = document.getElementById("hxList");
  if (!sel || !list) return;
  if (!_hxPages) {
    ...
    sel.innerHTML = _hxPages.length
      ? _hxPages
          .map(
            (p) =>
              `<option value="${p.id}">${escHtml(p.title || p.slug)} (${escHtml(p.slug)})</option>`
          )
          .join("")
  ...
  if (_hxPages.length) hxLoadVersions(+sel.value);
    ...
    list.innerHTML =
      '<p style="font-size:.72rem;color:var(--muted);font-weight:100;">Create a page first.</p>';
}
async function hxLoadVersions(pageId) {
  ...
  if (r.status === 403) {
    list.innerHTML =
      '<p style="font-size:.74rem;color:var(--muted);font-weight:200;line-height:1.7;">Version history is a <b style="color:#7fa6d8;">Pro</b> feature. Upgrade to access and restore past versions.</p>';
    return;
  }
  const vers = r.ok ? await r.json() : [];
  if (!Array.isArray(vers) || !vers.length) {
    list.innerHTML =
      '<p style="font-size:.74rem;color:var(--muted);font-weight:200;">No saved versions yet — they appear here after you edit and save this page.</p>';
    return;
  }
  list.innerHTML = vers
    .map(
      (v, i) =>
        `\n    <div class="hx-row" data-hxrow="${v.id}">\n      <div class="hx-row-main">\n        <div class="hx-row-title">${escHtml(v.title || "(untitled)")}</div>\n        <div class="hx-row-meta">${i === 0 ? '<b style="color:rgba(var(--accent-rgb),.8);font-weight:400;">latest</b><span class="hx-dot"></span>' : ""}${escHtml(timeAgo(v.created_at))}<span class="hx-dot"></span>${hxSize(v.size)}</div>\n      </div>\n      <button class="btn btn-xs" data-hxprev="${v.id}" data-pg="${pageId}">Preview</button>\n      <button class="btn btn-xs" data-hxrestore="${v.id}" data-pg="${pageId}">Restore</button>\n    </div>\n    <div class="hx-prev" id="hxprev-${v.id}" style="display:none;"></div>`
    )
    .join("");
  list
    .querySelectorAll("[data-hxrestore]")
    .forEach((b) =>
      b.addEventListener("click", () => hxRestore(+b.dataset.pg, +b.dataset.hxrestore))
    );
  list
    .querySelectorAll("[data-hxprev]")
    .forEach((b) => b.addEventListener("click", () => hxPreview(+b.dataset.pg, +b.dataset.hxprev)));
}
function hxSize(n) {
  n = +n || 0;
...
async function hxPreview(pageId, vid) {
  ...
  if (box.style.display !== "none") {
    ...
    row && row.classList.remove("open");
    return;
  }
  box.style.display = "block";
  row && row.classList.add("open");
  ...
  if (!v) {
    ...
    if (!r.ok) {
      box.innerHTML = '<span class="hx-dim">Could not load this version.</span>';
      return;
    }
    v = await r.json();
    _hxVerCache[vid] = v;
  ...
  let st = {};
  try {
    st = JSON.parse(v.page_json || "{}");
  } catch {}
  const secs = st.sections || [];
  if (!secs.length) {
    box.innerHTML = '<span class="hx-dim">Empty page.</span>';
    return;
  }
  const theme = {
    accent: st.accent || __SITE__.accent,
    text: st.text || __SITE__.text,
    bg: st.bg || __SITE__.bg,
    font: st.font || "Josefin Sans",
  };
  ...
  box.querySelectorAll(".hx-sec").forEach((b) =>
    b.addEventListener("click", () => {
      box.querySelectorAll(".hx-sec").forEach((x) => x.classList.remove("on"));
      b.classList.add("on");
      const s = secs[+b.dataset.si];
      try {
        prevArea.innerHTML = typeof bRender === "function" ? bRender(s, theme) : "";
      } catch (e) {
        prevArea.innerHTML =
          '<div style="padding:1rem;"><span class="hx-dim">Can’t preview this block type.</span></div>';
      }
    })
  );
}
async function hxRestore(pageId, vid) {
  if (
    !confirm(
      "Restore this version? Your current page content will be saved to history first, then replaced."
    )
  )
    return;
  const r = await fetch(`/api/pages/${pageId}/versions/${vid}/restore`, {
    method: "POST",
    headers: authHeaders(),
  });
  if (!r.ok) {
  ...
  if (typeof bldLoadPages === "function") {
    try {
      bldLoadPages();
    } catch {}
  }
...
async function loadAskIndexStatus() {
  const row = document.getElementById("askIndexRow"),
    stat = document.getElementById("askIndexStat");
  if (!row) return;
  try {
    ...
    if (d.enabled) {
      row.style.display = "";
      const ix = d.index || {};
      stat.textContent = ix.chunks
        ? `${ix.chunks} chunks from ${ix.pages} page${ix.pages === 1 ? "" : "s"}`
        : "not built yet";
    } else {
...
document.getElementById("askReindex")?.addEventListener("click", async (e) => {
  const btn = e.currentTarget,
    stat = document.getElementById("askIndexStat");
  btn.disabled = true;
  stat.textContent = "Rebuilding…";
  try {
    const r = await fetch("/api/ai/reindex", { method: "POST", headers: authHeaders() });
    const d = await r.json().catch(() => ({}));
    if (r.ok) {
      stat.textContent = `✓ ${d.chunks} chunks from ${d.pages} page${d.pages === 1 ? "" : "s"}`;
      toast("Search index rebuilt ✓");
    } else {
      stat.textContent = d.error || "failed";
      toast(d.error || "Reindex failed.", true);
    }
  } catch {
    stat.textContent = "failed";
    toast("Reindex failed.", true);
  }
  btn.disabled = false;
});
function _pVapidBytes(b64) {
...
async function loadPushAdmin() {
  const g = document.getElementById("pushGroup");
  if (!g) return;
  try {
    const d = await fetch("/api/push/stats", { headers: authHeaders() }).then((r) => r.json());
    if (d.enabled) {
      g.style.display = "";
      const s = document.getElementById("pushStat");
      if (s)
        s.textContent = `${d.visitors} subscriber${d.visitors === 1 ? "" : "s"} · ${d.owners} owner device${d.owners === 1 ? "" : "s"}`;
...
async function loadInbox() {
  ...
  try {
    data = await fetch("/api/inbox", { headers: authHeaders() }).then((r) =>
      r.ok ? r.json() : { messages: [], unread: 0 }
    );
  } catch (e) {}
  const msgs = data.messages || [];
  const badge = document.getElementById("inboxBadge");
  if (badge) badge.textContent = data.unread ? data.unread : "";
  if (!msgs.length) {
    w.innerHTML = `<h1 style="font-weight:200;font-size:1.3rem;color:var(--white);">Inbox</h1><p style="font-weight:200;font-size:.8rem;color:var(--muted);margin-top:.6rem;">No messages yet. Contact-form submissions land here.</p>`;
    return;
  }
  w.innerHTML =
    `<h1 style="font-weight:200;font-size:1.3rem;color:var(--white);margin:0 0 1rem;">Inbox <span style="font-size:.8rem;color:var(--muted);">· ${msgs.length}</span></h1>` +
    msgs
      .map(
        (m) =>
  ...
  msgs
    .filter((m) => !m.read_at)
    .forEach((m) =>
      fetch(`/api/inbox/${m.id}/read`, { method: "POST", headers: authHeaders() }).catch(() => {})
    );
  if (badge) badge.textContent = "";
  w.querySelectorAll("[data-mdel]").forEach((b) =>
    b.addEventListener("click", async () => {
      if (!(await dlg.confirm("Delete this message?", { danger: true, confirm: "Delete" }))) return;
      const r = await fetch(`/api/inbox/${b.dataset.mdel}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      if (r.ok) {
        w.querySelector(`[data-msg="${b.dataset.mdel}"]`)?.remove();
      }
    })
  );
}
async function refreshInboxBadge() {
  ...
  try {
    const d = await fetch("/api/inbox", { headers: authHeaders() }).then((r) =>
      r.ok ? r.json() : null
    );
    const b = document.getElementById("inboxBadge");
    if (b && d) b.textContent = d.unread ? d.unread : "";
  } catch (e) {}
}
async function loadHome() {
  const w = document.getElementById("homeWrap");
  if (!w) return;
  const A = () => ({ headers: authHeaders() });
  const [pages, tuts, revs, comments, settings, an, cfg] = await Promise.all([
    fetch("/api/nav/pages", A())
      .then((r) => (r.ok ? r.json() : []))
      .catch(() => []),
    fetch("/api/tutorials", A())
      .then((r) => (r.ok ? r.json() : []))
      .catch(() => []),
    fetch("/api/reviews", A())
      .then((r) => (r.ok ? r.json() : []))
      .catch(() => []),
    fetch("/api/comments/admin", A())
      .then((r) => (r.ok ? r.json() : []))
      .catch(() => []),
    fetch("/api/settings")
      .then((r) => (r.ok ? r.json() : {}))
      .catch(() => ({})),
    fetch("/api/analytics", A())
  ...
  const plan = (cfg && cfg.plan) || "free";
  window.foyerPlan = plan;
  const ultra = plan === "ultra";
  const len = (a) => (Array.isArray(a) ? a.length : 0);
  const num = (o) => (o && typeof o === "object" ? o.n || 0 : o || 0);
  const today = an ? num(an.today) : 0,
    week = an ? num(an.week) : 0;
  const isPublic = settings.site_public === "1",
    offline = settings.site_offline === "1";
  const pill = (txt, col) =>
    `<span style="font-size:.58rem;font-weight:500;letter-spacing:.08em;text-transform:uppercase;padding:.2rem .55rem;border-radius:20px;background:${col}22;color:${col};border:1px solid ${col}55;">${txt}</span>`;
  const card = (label, val, go) =>
    `<button type="button" ${go ? `data-go="${go}"` : ""} class="home-card" style="text-align:left;background:var(--panel);border:1px solid var(--border);border-radius:12px;padding:1.05rem 1.15rem;cursor:${go ? "pointer" : "default"};">\n    <div style="font-size:1.75rem;font-weight:200;color:var(--white);line-height:1;">${val}</div>\n    <div style="font-size:.6rem;font-weight:300;letter-spacing:.12em;text-transform:uppercase;color:var(--muted);margin-top:.45rem;">${label}</div></button>`;
  const recent = (Array.isArray(comments) ? comments : []).slice(0, 5);
  const recentHtml = recent.length
    ? recent
        .map(
          (c) =>
            `<div style="display:flex;gap:.7rem;align-items:baseline;padding:.55rem 0;border-top:1px solid var(--border);">\n      <div style="font-size:.76rem;color:var(--white);flex-shrink:0;"><b style="font-weight:400;">${escHtml(c.name || "")}</b></div>\n      <div style="flex:1;min-width:0;font-size:.74rem;font-weight:200;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escHtml(c.body || "")}</div>\n      <div style="font-size:.6rem;color:var(--muted);opacity:.6;flex-shrink:0;">${timeAgo(c.created_at)}</div></div>`
        )
        .join("")
  ...
  w.querySelectorAll("[data-go]").forEach((b) =>
    b.addEventListener("click", () =>
      document.querySelector(`.tab-btn[data-tab="${b.dataset.go}"]`)?.click()
    )
  );
  w.querySelectorAll("[data-ext]").forEach((b) =>
    b.addEventListener("click", () => window.open(b.dataset.ext, "_blank"))
  );
...
document.getElementById("pushOwner")?.addEventListener("click", async () => {
  try {
    if (!("serviceWorker" in navigator) || !("PushManager" in window)) {
      toast("Push not supported in this browser.", true);
      return;
    }
    const cfg = await fetch("/api/push/config")
      .then((r) => r.json())
      .catch(() => ({}));
    if (!cfg.enabled || !cfg.vapid_public) {
      toast("Push isn't configured.", true);
      return;
    }
    if (
    ...
    if (!sub)
      sub = await reg.pushManager.subscribe({
        ...
        applicationServerKey: _pVapidBytes(cfg.vapid_public),
      });
    const r = await fetch("/api/push/subscribe", {
      method: "POST",
      headers: { ...authHeaders(), "Content-Type": "application/json" },
      body: JSON.stringify({ subscription: sub.toJSON(), kind: "owner" }),
    });
    toast(r.ok ? "Owner alerts enabled on this device ✓" : "Failed.", !r.ok);
    loadPushAdmin();
  } catch (e) {
    toast("Could not enable: " + (e.message || e), true);
  }
});
...
document.getElementById("pushSend")?.addEventListener("click", async (e) => {
  const btn = e.currentTarget,
    stat = document.getElementById("pushSendStat");
  ...
  toast.update(
    ...
    r.ok
      ? {
          type: "success",
          message: `Sent to ${d.sent || 0} subscriber${d.sent === 1 ? "" : "s"}.`,
          autoClose: 4e3,
        }
      : { type: "error", message: d.error || "Failed to send.", autoClose: 4e3 }
  );
});
window.foyerPlan = "free";
window.foyerPerms = "all";
function foyerCan(p) {
  return (
    window.foyerPerms === "all" ||
    (Array.isArray(window.foyerPerms) && window.foyerPerms.includes(p))
  );
}
const TAB_PERM = {
  builder: "pages",
...
function applyTabVisibility() {
  ...
  Object.keys(TAB_PERM).forEach((t) => {
    ...
    if (!b) return;
    if (t === "inbox") {
      b.style.display = ultra && foyerCan("inbox") ? "" : "none";
      return;
    }
...
fetch("/api/config")
  ...
  .then((c) => {
    ...
    if (
      typeof bldDrawPages === "function" &&
      document.getElementById("bldPageList")?.children.length
    )
      bldDrawPages();
    if (window.foyerPlan === "ultra" && typeof refreshInboxBadge === "function")
      refreshInboxBadge();
    applyTabVisibility();
  })
  .catch(() => {});
