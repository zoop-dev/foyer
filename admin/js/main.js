import { authHeaders, dlg, escAttr, escHtml, init, pushUiUpdate, timeAgo, toast } from "./utils.js";
import { fetchAnalytics } from "./analytics.js";
import { fetchTutorials } from "./tutorials.js";
import { fetchReviews } from "./reviews.js";
import { fetchColls } from "./collections.js";
import { renderBackupTab } from "./backup.js";
window.openImgPicker = openImgPicker;
...
function renderTabToggles() {
  ...
  el.innerHTML = HIDEABLE_TABS.map(
    (t) => `<label style="display:flex;align-items:center;gap:.6rem;cursor:pointer;font-size:.72rem;font-weight:200;color:rgba(220,245,225,.8);padding:.25rem 0;"><input type="checkbox" data-tabtoggle="${t}" ${hidden.includes(t) ? "" : "checked"} /> ${TAB_LABELS[t]}</label>`
  ).join("");
...
export async function fetchSettings() {
  ...
  await buildLangPicker(
    (s.site_languages || "").split(",").map((x) => x.trim().toLowerCase()).filter(Boolean)
  );
  loadPushAdmin();
...
async function buildLangPicker(selected) {
  ...
  base.innerHTML = _i18nCodes.map(
    (c) => `<option value="${c}"${c === baseSel ? " selected" : ""}>${name(c)} (${c})</option>`
  ).join("");
  opts.innerHTML = _i18nCodes.filter((c) => c !== baseSel).map(
    (c) => `<label style="display:flex;align-items:center;gap:.35rem;cursor:pointer;font-size:.7rem;font-weight:200;color:rgba(220,245,225,.8);"><input type="checkbox" data-lang="${c}" ${extras.has(c) ? "checked" : ""} style="width:auto;" /> ${name(c)} <span style="opacity:.45;">${c}</span></label>`
  ).join("");
  base.onchange = () => buildLangPicker([
    base.value,
    ...[...opts.querySelectorAll("input[data-lang]:checked")].map((i) => i.dataset.lang).filter((c) => c !== base.value)
  ]);
}
...
function renderNavPageList(pages) {
  ...
  if (!el) return;
  if (!pages.length) {
    el.innerHTML = '<p style="font-size:.62rem;color:var(--muted);font-weight:100;">No published pages yet.</p>';
    return;
  }
  el.innerHTML = pages.map(
    ...
    <div class="nav-page-row" data-pid="${p.id}" data-idx="${i}" draggable="true" style="display:flex;align-items:center;gap:.6rem;padding:.45rem .6rem;border:1px solid var(--border);background:var(--surface);cursor:grab;">
      ...
      <select data-parent-pid="${p.id}" title="Sub-page of…" style="font-size:.58rem;font-weight:200;background:var(--bg);color:rgba(220,245,225,.8);border:1px solid var(--border);padding:.25rem .35rem;max-width:130px;">
        ...
        ${pages.filter((o) => o.id !== p.id).map(
      (o) => `<option value="${escAttr(o.slug)}" ${p.parent === o.slug ? "selected" : ""}>↳ ${escHtml(o.title)}</option>`
    ).join("")}
      </select>
...
function renderNavCustomLinks() {
  ...
  if (!_navCustomLinks.length) {
    ...
    el.innerHTML = _navCustomLinks.map(
      (l, i) => `
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      ov.innerHTML = `
        ...
        ([v, l], i) => `<button class="crop-ar${i === 0 ? " on" : ""}" data-ar="${v}" style="font-family:inherit;font-size:.58rem;letter-spacing:.1em;text-transform:uppercase;font-weight:200;padding:.35rem .7rem;border:1px solid var(--border);background:${i === 0 ? "rgba(var(--accent-rgb),.18)" : "transparent"};color:var(--white);cursor:pointer;">${l}</button>`
      ).join("")}
        </div>
      ...
      function place(h, x, y) {
        ...
        box.querySelector(`[data-h="${h}"]`).style.cssText += `;${{ nw: `left:${m}px;top:${m}px;cursor:nwse-resize`, ne: `right:${m}px;top:${m}px;cursor:nesw-resize`, sw: `left:${m}px;bottom:${m}px;cursor:nesw-resize`, se: `right:${m}px;bottom:${m}px;cursor:nwse-resize`, n: `left:${mid};top:${m}px;cursor:ns-resize`, s: `left:${mid};bottom:${m}px;cursor:ns-resize`, e: `right:${m}px;top:${mid};cursor:ew-resize`, w: `left:${m}px;top:${mid};cursor:ew-resize` }[h]}`;
      }
      ...
      if (elImg.complete && elImg.naturalWidth) requestAnimationFrame(init2);
      else elImg.onload = () => requestAnimationFrame(init2);
    };
    img.onerror = () => {
      URL.revokeObjectURL(url);
      resolve(file);
...
export function renderImgTabGallery() {
  ...
  gallery.innerHTML = _imgList.map(
    (img) => `
...
function bustImgCache(id) {
  try {
    if (window.caches)
      caches.open("foyer-img-v1").then((c) => c.delete("/api/images/" + id)).catch(() => {
      });
  } catch (e) {
  }
}
...
export function openImgPicker(callback) {
  ...
  grid.innerHTML = _imgList.length ? _imgList.map(
    (img) => `<div class="img-picker-item" data-id="${img.id}" title="${escHtml(img.name || "")}"><img src="/api/images/${img.id}" alt="" /><div class="img-picker-item-name">${escHtml(img.name || "untitled")}</div><div style="font-size:.5rem;color:rgba(77,189,106,.35);padding:0 .4rem .35rem;font-family:monospace;">/api/images/${img.id}</div></div>`
  ).join("") : '<p style="font-weight:200;font-size:.68rem;color:var(--muted);padding:1rem;">No images yet — go to the Images tab to upload.</p>';
...
function openFilePicker(callback) {
  ...
  fetch("/api/files", { headers: authHeaders() }).then((r) => r.json()).then((files) => {
    if (!files.length) {
      list.innerHTML = '<p style="font-weight:200;font-size:.68rem;color:var(--muted);padding:1rem;">No files yet — go to the Files tab to upload.</p>';
      return;
    }
    list.innerHTML = files.map(
      (f) => `<div class="file-pick-row" data-furl="/api/files/${f.id}"><span style="font-size:1.1rem;">${fileIcon(f.mime)}</span><span class="file-pick-row-name">${escHtml(f.name)}</span><span class="file-pick-row-meta">${fmtBytes(f.size)}</span></div>`
    ).join("");
    ...
    list.innerHTML = '<p style="color:var(--muted);padding:1rem;font-size:.7rem;">Error loading files.</p>';
  });
  overlay.classList.add("open");
}
...
async function fetchAllowlist() {
  ...
  el.querySelectorAll(".al-remove-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      if (!await dlg.confirm(`Remove ${btn.dataset.email} from the allowlist?`, {
        confirm: "Remove"
      }))
        return;
...
document.querySelectorAll(".set-cat").forEach(
...
async function renderHistoryTab() {
  ...
  if (!_hxPages) {
    ...
    sel.innerHTML = _hxPages.length ? _hxPages.map(
      (p) => `<option value="${p.id}">${escHtml(p.title || p.slug)} (${escHtml(p.slug)})</option>`
  ...
  if (_hxPages.length) hxLoadVersions(+sel.value);
    ...
    list.innerHTML = '<p style="font-size:.72rem;color:var(--muted);font-weight:100;">Create a page first.</p>';
}
async function hxLoadVersions(pageId) {
  ...
  if (r.status === 403) {
    list.innerHTML = '<p style="font-size:.74rem;color:var(--muted);font-weight:200;line-height:1.7;">Version history is a <b style="color:#7fa6d8;">Pro</b> feature. Upgrade to access and restore past versions.</p>';
    return;
  }
  const vers = r.ok ? await r.json() : [];
  if (!Array.isArray(vers) || !vers.length) {
    list.innerHTML = '<p style="font-size:.74rem;color:var(--muted);font-weight:200;">No saved versions yet — they appear here after you edit and save this page.</p>';
    return;
  }
  list.innerHTML = vers.map(
    (v, i) => `
  ...
  list.querySelectorAll("[data-hxrestore]").forEach(
    (b) => b.addEventListener("click", () => hxRestore(+b.dataset.pg, +b.dataset.hxrestore))
  );
...
async function hxPreview(pageId, vid) {
  ...
  box.querySelectorAll(".hx-sec").forEach(
    (b) => b.addEventListener("click", () => {
      ...
      try {
        ...
        prevArea.innerHTML = '<div style="padding:1rem;"><span class="hx-dim">Can’t preview this block type.</span></div>';
      }
    })
  );
}
...
async function loadCommentsAdmin() {
  ...
  let rows = [];
  try {
    rows = await fetch("/api/comments/admin", { headers: authHeaders() }).then(
      (r) => r.ok ? r.json() : []
    );
  } catch {
  }
  ...
  el.innerHTML = rows.map(
    (c) => `<div data-crow="${c.id}" style="display:flex;gap:.6rem;align-items:flex-start;padding:.5rem 0;border-top:1px solid var(--border);">
  ...
  el.querySelectorAll(".cmtDel").forEach(
...
async function loadInbox() {
  ...
  try {
    data = await fetch("/api/inbox", { headers: authHeaders() }).then(
      (r) => r.ok ? r.json() : { messages: [], unread: 0 }
    );
  } catch (e) {
  }
  ...
  w.innerHTML = `<h1 style="font-weight:200;font-size:1.3rem;color:var(--white);margin:0 0 1rem;">Inbox <span style="font-size:.8rem;color:var(--muted);">· ${msgs.length}</span></h1>` + msgs.map(
    (m) => `
  ...
  msgs.filter((m) => !m.read_at).forEach(
    (m) => fetch(`/api/inbox/${m.id}/read`, { method: "POST", headers: authHeaders() }).catch(() => {
    })
  );
  if (badge) badge.textContent = "";
  w.querySelectorAll("[data-mdel]").forEach(
    (b) => b.addEventListener("click", async () => {
...
async function refreshInboxBadge() {
  ...
  try {
    const d = await fetch("/api/inbox", { headers: authHeaders() }).then(
      (r) => r.ok ? r.json() : null
    );
...
async function loadHome() {
  ...
  const recentHtml = recent.length ? recent.map(
    (c) => `<div style="display:flex;gap:.7rem;align-items:baseline;padding:.55rem 0;border-top:1px solid var(--border);">
  ...
  ).join("") : `<p style="font-size:.72rem;font-weight:200;color:var(--muted);opacity:.7;padding:.4rem 0;">No comments yet.</p>`;
  w.innerHTML = `
  ...
  w.querySelectorAll("[data-go]").forEach(
    (b) => b.addEventListener(
      "click",
      () => document.querySelector(`.tab-btn[data-tab="${b.dataset.go}"]`)?.click()
    )
  );
  w.querySelectorAll("[data-ext]").forEach(
    (b) => b.addEventListener("click", () => window.open(b.dataset.ext, "_blank"))
  );
...
document.getElementById("pushOwner")?.addEventListener("click", async () => {
  try {
    ...
    const cfg = await fetch("/api/push/config").then((r2) => r2.json()).catch(() => ({}));
...
function foyerCan(p) {
  return window.foyerPerms === "all" || Array.isArray(window.foyerPerms) && window.foyerPerms.includes(p);
}
const TAB_PERM = {
  builder: "pages",
  images: "media",
...
fetch("/api/config").then((r) => r.ok ? r.json() : {}).then((c) => {
  ...
  if (typeof bldDrawPages === "function" && document.getElementById("bldPageList")?.children.length)
    bldDrawPages();
...
fetch("/api/me", { headers: authHeaders() }).then((r) => r.ok ? r.json() : null).then((m) => {
  if (m && m.perms) {
