...
function bRender(s, theme) {
  const {
    accent: accent = __SITE__.accent,
    text: text = __SITE__.text,
  ...
  switch (s.type) {
      ...
      const cards = items
        .map((it) => {
          ...
          const coverHtml = cover
            ? `<img src="${bA(cover)}" alt="" style="width:100%;height:130px;object-fit:cover;display:block;" />`
            : `<div style="width:100%;height:130px;background:${bRgb(accent, 0.06)};border:1px dashed ${bRgb(accent, 0.15)};"></div>`;
          const thumbs =
            imgs.length > 1
              ? `<div style="display:flex;gap:.25rem;padding:.35rem .35rem 0;overflow-x:auto;">${imgs.map((u) => `<img src="${bA(u)}" alt="" style="width:30px;height:30px;object-fit:cover;flex-shrink:0;border:1px solid ${bRgb(accent, 0.2)};" />`).join("")}</div>`
              : "";
          const onSale =
            it.for_sale === "yes" &&
            it.sale === "yes" &&
          ...
          const badge =
            it.for_sale === "yes"
              ? `<div style="position:absolute;top:.5rem;right:.5rem;${onSale ? "background:#e0556a;color:#fff;" : `background:${accent};color:${bg};`}font-size:.5rem;font-weight:400;letter-spacing:.1em;text-transform:uppercase;padding:.2rem .5rem;z-index:1;">${onSale ? `Sale · <span style="text-decoration:line-through;opacity:.7;">${bE(it.price || "")}</span> ${bE(it.sale_price)}` : `For sale${it.price ? ` · ${bE(it.price)}` : ""}`}</div>`
              : "";
          return `<div style="border:1px solid ${bRgb(accent, 0.12)};background:${bRgb(accent, 0.03)};position:relative;">${badge}${coverHtml}${thumbs}<div style="padding:.6rem .8rem;"><div style="font-weight:300;font-size:.82rem;color:${bRgb(text, 0.9)};">${bE(it.name || "")}</div>${it.note ? `<div style="font-weight:200;font-size:.68rem;line-height:1.6;color:${bRgb(text, 0.5)};margin-top:.2rem;">${bE(it.note)}</div>` : ""}${it.buy === "yes" && it.buy_url ? `<a href="${bA(it.buy_url)}" target="_blank" rel="noopener" style="display:inline-block;margin-top:.55rem;background:${accent};color:${bg};font-size:.62rem;font-weight:400;letter-spacing:.08em;text-transform:uppercase;padding:.4rem .8rem;border-radius:6px;text-decoration:none;">${bE(it.buy_label || "Buy")}</a>` : ""}</div></div>`;
        })
        .join("");
      return `<div style="${ff}${fc}padding:${p};">${s.heading ? `<div style="font-weight:300;font-size:1.05rem;letter-spacing:.02em;margin-bottom:.3rem;">${bE(s.heading)}</div>` : ""}${countLine ? `<div style="font-size:.65rem;font-weight:200;letter-spacing:.1em;text-transform:uppercase;color:${bRgb(accent, 0.6)};margin-bottom:1.2rem;">${countLine}</div>` : ""}<div style="display:grid;grid-template-columns:repeat(${cols},1fr);gap:1rem;">${cards}</div></div>`;
    }
    case "hero": {
    ...
    case "bio": {
      ...
      const photoHtml = s.photo
        ? `<img src="${bA(s.photo)}" alt="" style="width:${s.photo_size || "80px"};height:${s.photo_size || "80px"};object-fit:cover;border-radius:${pr};display:block;${s.align === "center" ? "margin:0 auto 1.2rem;" : "margin-bottom:1.2rem;"}" />`
        : "";
      return `<div style="${ff}${fc}${ta}padding:${p};">\n        ${photoHtml}\n        ${s.heading ? `<h2 style="font-size:1.35rem;font-weight:300;letter-spacing:.04em;margin-bottom:1rem;">${bE(s.heading)}</h2>` : ""}\n        <div class="md-content" style="font-weight:200;font-size:${fs};line-height:${lh};color:${bRgb(text, 0.72)};max-width:${mw};${mc}">${md(s.body)}</div>\n      </div>`;
    }
    case "links": {
      const items = s.items || [];
      const p = s.pad === "sm" ? "1rem 2rem" : s.pad === "lg" ? "4rem 2rem" : "2rem 2rem";
      ...
      if (ls === "minimal") {
        inner = items
          .map(
            (it) =>
              `<a href="${bA(it.u || "#")}" ${itTarget(it)} style="display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid ${bRgb(accent, 0.12)};padding:.75rem 0;text-decoration:none;"><span style="font-weight:300;font-size:.9rem;letter-spacing:.06em;color:${bRgb(text, 0.88)};">${bE(it.t || "Link")}</span>${it.d ? `<span style="font-size:.62rem;color:${bRgb(accent, 0.42)};">${bE(it.d)}</span>` : ""}</a>`
          )
          .join("");
      } else if (ls === "pill") {
        inner = `<div style="display:flex;flex-direction:column;gap:.5rem;">${items.map((it) => `<a href="${bA(it.u || "#")}" ${itTarget(it)} style="display:block;border:1px solid ${bRgb(accent, 0.18)};background:${bRgb(accent, 0.04)};padding:.8rem 1.4rem;border-radius:40px;text-decoration:none;"><div style="font-weight:300;font-size:.9rem;letter-spacing:.06em;color:${bRgb(text, 0.88)};">${bE(it.t || "Link")}</div>${it.d ? `<div style="font-size:.62rem;font-weight:200;color:${bRgb(accent, 0.42)};margin-top:.15rem;">${bE(it.d)}</div>` : ""}</a>`).join("")}</div>`;
      } else {
        inner = `<div style="display:flex;flex-direction:column;gap:.52rem;">${items.map((it) => `<a href="${bA(it.u || "#")}" ${itTarget(it)} style="display:block;border:1px solid ${bRgb(accent, 0.15)};background:${bRgb(accent, 0.03)};padding:.85rem 1.2rem;text-decoration:none;"><div style="font-weight:300;font-size:.9rem;letter-spacing:.06em;color:${bRgb(text, 0.88)};">${bE(it.t || "Link")}</div>${it.d ? `<div style="font-size:.62rem;font-weight:200;color:${bRgb(accent, 0.42)};margin-top:.18rem;">${bE(it.d)}</div>` : ""}</a>`).join("")}</div>`;
      }
    ...
    case "contact": {
      ...
      const subLine = (sub) =>
        sub
          ? `<span style="display:block;font-size:.62rem;font-weight:200;letter-spacing:.06em;color:${bRgb(accent, 0.42)};margin-top:.12rem;">${bE(sub)}</span>`
          : "";
      if (cst === "stacked") {
        inner = rawItems
          .map(
            (it) =>
              `<div style="display:flex;flex-direction:column;gap:.2rem;"><span style="font-size:.55rem;letter-spacing:.3em;text-transform:uppercase;color:${bRgb(accent, 0.4)};font-weight:200;">${bE(it.label || "")}</span><span style="font-size:.95rem;font-weight:300;letter-spacing:.06em;color:${bRgb(text, 0.88)};">${bE(it.val || "")}</span>${subLine(it.sub)}</div>`
          )
          .join("");
      } else if (cst === "minimal") {
        inner = rawItems
          .map(
            (it) =>
              `<div><a href="${bA(it.href || "")}" style="font-size:.95rem;font-weight:300;letter-spacing:.06em;color:${bRgb(text, 0.82)};text-decoration:none;">${bE(it.val || "")}</a>${subLine(it.sub)}</div>`
          )
          .join("");
      } else {
        inner = rawItems
          .map(
            (it) =>
              `<div style="display:flex;align-items:baseline;gap:1rem;"><span style="font-size:.65rem;letter-spacing:.26em;text-transform:uppercase;color:${bRgb(accent, 0.38)};min-width:3rem;text-align:right;font-weight:200;">${bE(it.label || "")}</span><div><a href="${bA(it.href || "")}" style="font-size:.92rem;font-weight:300;letter-spacing:.06em;color:${bRgb(text, 0.85)};text-decoration:none;">${bE(it.val || "")}</a>${subLine(it.sub)}</div></div>`
          )
          .join("");
      }
      ...
      const btnCss =
        bs === "solid"
          ...
          : bs === "minimal"
            ...
            : bs === "pill"
              ? `background:none;border:1px solid ${bRgb(accent, 0.25)};color:${accent};border-radius:40px;padding:.4rem .9rem;`
              : `background:none;border:1px solid ${bRgb(accent, 0.22)};color:${accent};padding:.4rem .9rem;`;
    ...
    case "cta": {
      ...
      const btnCss =
        bs === "outline"
          ...
          : bs === "ghost"
            ? `background:transparent;border:1px solid ${bRgb(text, 0.25)};color:${text};`
            : `background:${accent};border:1px solid ${accent};color:${bg};`;
      return `<div style="${ff}${fc}${ta}padding:${p};">\n        ${s.text ? `<p style="font-weight:200;font-size:1rem;letter-spacing:.06em;margin-bottom:1.4rem;color:${bRgb(text, 0.75)};">${bE(s.text)}</p>` : ""}\n        <a href="${bA(s.button_url || "#")}" style="display:inline-block;font-weight:300;font-size:.78rem;letter-spacing:.25em;text-transform:uppercase;padding:${bsz};text-decoration:none;${btnCss}">${bE(s.button_label || "Get in touch")}</a>\n      </div>`;
    }
    case "quote": {
      const p = s.pad === "sm" ? "1.5rem 2rem" : s.pad === "lg" ? "5rem 2rem" : "3rem 2rem";
      const v = s.variant || "border";
      if (v === "centered")
        return `<div style="${ff}${fc}padding:${p};text-align:center;"><div style="max-width:480px;margin:0 auto;"><p style="font-size:2.5rem;font-weight:200;line-height:1;color:${bRgb(accent, 0.3)};margin-bottom:.5rem;">"</p><p style="font-size:1rem;font-weight:200;line-height:1.85;color:${bRgb(text, 0.75)};font-style:italic;">${bE(s.quote || "")}</p>${s.attribution ? `<cite style="display:block;margin-top:.8rem;font-size:.67rem;font-weight:200;letter-spacing:.2em;text-transform:uppercase;color:${bRgb(accent, 0.45)};font-style:normal;">— ${bE(s.attribution)}</cite>` : ""}</div></div>`;
      if (v === "minimal")
        return `<div style="${ff}${fc}padding:${p};max-width:480px;margin:0 auto;"><p style="font-size:1rem;font-weight:200;line-height:1.85;color:${bRgb(text, 0.72)};font-style:italic;">${bE(s.quote || "")}</p>${s.attribution ? `<cite style="display:block;margin-top:.6rem;font-size:.67rem;font-weight:200;letter-spacing:.2em;text-transform:uppercase;color:${bRgb(accent, 0.38)};font-style:normal;">— ${bE(s.attribution)}</cite>` : ""}</div>`;
      return `<div style="${ff}${fc}padding:${p};text-align:center;"><blockquote style="border-left:2px solid ${bRgb(accent, 0.4)};padding:1rem 1.5rem;text-align:left;max-width:440px;margin:0 auto;"><p style="font-size:1rem;font-weight:200;font-style:italic;line-height:1.85;color:${bRgb(text, 0.75)};">"${bE(s.quote || "Your quote")}"</p>${s.attribution ? `<cite style="display:block;margin-top:.7rem;font-size:.67rem;font-weight:200;letter-spacing:.2em;text-transform:uppercase;color:${bRgb(accent, 0.45)};font-style:normal;">— ${bE(s.attribution)}</cite>` : ""}</blockquote></div>`;
    }
    case "heading": {
    ...
    case "text": {
      ...
      return `<div style="${ff}${fc}${ta}padding:${p};"><div class="md-content" style="font-weight:200;font-size:${fs};line-height:${lh};color:${bRgb(text, 0.72)};max-width:${mw};${mc}">${md(s.text)}</div></div>`;
    }
    case "image": {
      const ta = `text-align:${s.align || "center"};`;
        ...
        s.radius === "sm"
          ...
          : s.radius === "lg"
            ...
            : s.radius === "circle"
              ...
              : "0";
      const p = s.pad === "sm" ? "1rem 2rem" : s.pad === "lg" ? "4rem 2rem" : "2rem 2rem";
      return `<div style="${ff}${fc}${ta}padding:${p};">${s.url ? `<img src="${bA(s.url)}" alt="${bA(s.caption || "")}" style="max-width:${mw};width:100%;height:auto;display:inline-block;border-radius:${r};" />` : `<div style="display:inline-block;width:100%;max-width:${mw};background:${bRgb(accent, 0.06)};border:1px dashed ${bRgb(accent, 0.2)};padding:3rem 2rem;font-size:.68rem;letter-spacing:.2em;color:${bRgb(accent, 0.3)};border-radius:${r};">Image URL</div>`}${s.caption ? `<p style="margin-top:.55rem;font-size:.62rem;font-weight:200;letter-spacing:.14em;color:${bRgb(text, 0.38)};">${bE(s.caption)}</p>` : ""}</div>`;
    }
    case "stats": {
      const items = s.items || [];
      const p = s.pad === "sm" ? "1.5rem 2rem" : s.pad === "lg" ? "4.5rem 2rem" : "2.5rem 2rem";
      ...
      return `<div style="${ff}${fc}padding:${p};text-align:center;"><div style="${gs}">${items.map((it) => `<div><div style="font-size:clamp(2rem,6vw,3rem);font-weight:300;letter-spacing:-.02em;color:${accent};">${bE(it.number || "0")}</div><div style="font-size:.6rem;font-weight:200;letter-spacing:.3em;text-transform:uppercase;color:${bRgb(text, 0.5)};margin-top:.3rem;">${bE(it.label || "")}</div></div>`).join("")}</div></div>`;
    }
    case "divider": {
      const sp = s.spacing === "sm" ? ".8rem" : s.spacing === "lg" ? "3rem" : "1.8rem";
      const dbg =
        s.style === "fade"
          ? `linear-gradient(90deg,transparent,${bRgb(accent, 0.38)},transparent)`
          : bRgb(accent, 0.28);
    ...
    case "link": {
      ...
      if (sty === "solid") bc = `background:${accent};color:${bg};border:none;`;
        ...
        bc = `background:transparent;border:1px solid ${bRgb(text, 0.25)};color:${text};`;
      else if (sty === "underline")
        bc = `background:none;border:none;border-bottom:1px solid ${bRgb(accent, 0.5)};color:${accent};padding-bottom:.2rem;border-radius:0;`;
      else if (sty === "badge")
        bc = `background:${bRgb(accent, 0.12)};border:1px solid ${bRgb(accent, 0.25)};color:${accent};`;
      else
        bc = `background:none;border:1px solid ${bRgb(accent, 0.3)};color:${accent};border-radius:40px;`;
    ...
    case "cards": {
      ...
      const cImg = (it, h = "140px") =>
        it.img
          ...
          : `<div style="width:100%;height:${h};background:${bRgb(accent, 0.06)};border:1px dashed ${bRgb(accent, 0.15)};border-radius:${r};display:flex;align-items:center;justify-content:center;font-size:.52rem;letter-spacing:.15em;color:${bRgb(accent, 0.25)};">image</div>`;
      const cText = (it) =>
        it.title || it.body
          ? `<div style="padding:.65rem 0;">${it.title ? `<div style="font-weight:300;font-size:.85rem;letter-spacing:.04em;color:${bRgb(text, 0.9)};margin-bottom:.25rem;">${bE(it.title)}</div>` : ""} ${it.body ? `<div style="font-weight:200;font-size:.76rem;line-height:1.7;color:${bRgb(text, 0.52)};">${bE(it.body)}</div>` : ""}</div>`
          : " ";
      const cOpen = (it, sty = "") =>
        it.url
          ? `<a href="${bA(it.url || "#")}" ${it.new_tab !== "no" ? 'target="_blank" rel="noopener"' : ""} style="${sty}display:block;text-decoration:none;">`
          : `<div style="${sty}">`;
      ...
      if (layout === "grid-2") {
        ...
        inner = `<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.7rem;">${items.map((it) => `${cOpen(it)}${cImg(it, "80px")}${it.title ? `<div style="font-weight:300;font-size:.7rem;letter-spacing:.04em;color:${bRgb(text, 0.85)};margin-top:.38rem;">${bE(it.title)}</div>` : " "}${cClose(it)}`).join("")}</div>`;
        ...
        inner = items
          .map(
            (it) =>
              `${cOpen(it, `display:grid;grid-template-columns:2fr 3fr;gap:1.2rem;align-items:center;padding:.75rem;border:1px solid ${bRgb(accent, 0.09)};margin-bottom:.65rem;`)}${cImg(it, "100px")}<div>${it.title ? `<div style="font-weight:300;font-size:.88rem;letter-spacing:.04em;color:${bRgb(text, 0.9)};margin-bottom:.28rem;">${bE(it.title)}</div>` : ""} ${it.body ? `<div style="font-weight:200;font-size:.76rem;line-height:1.7;color:${bRgb(text, 0.52)};">${bE(it.body)}</div>` : ""}</div>${cClose(it)}`
          )
          .join("");
      } else {
        inner = `<div style="display:flex;flex-direction:column;">${items.map((it) => `${cOpen(it, `display:flex;gap:.9rem;align-items:center;padding:.55rem 0;border-bottom:1px solid ${bRgb(accent, 0.07)};`)} <div style="flex-shrink:0;width:68px;height:50px;overflow:hidden;border-radius:${r};">${it.img ? `<img src="${bA(it.img)}" alt="" style="width:100%;height:100%;object-fit:cover;" />` : `<div style="width:100%;height:100%;background:${bRgb(accent, 0.06)};border:1px dashed ${bRgb(accent, 0.12)};"></div>`}</div><div style="flex:1;min-width:0;">${it.title ? `<div style="font-weight:300;font-size:.82rem;letter-spacing:.04em;color:${bRgb(text, 0.88)};margin-bottom:.15rem;">${bE(it.title)}</div>` : ""} ${it.body ? `<div style="font-weight:200;font-size:.7rem;line-height:1.6;color:${bRgb(text, 0.5)};">${bE(it.body)}</div>` : ""}</div>${cClose(it)}`).join("")}</div>`;
      }
      return `<div style="${ff}${fc}padding:${p};">${inner}</div>`;
    }
    case "gallery": {
      ...
      const inner = `<div style="display:grid;grid-template-columns:repeat(${cols},1fr);gap:${gap};">${items
        .map((it) => {
          const wrap = it.url
            ? `<a href="${bA(it.url)}" target="_blank" rel="noopener" style="display:block;overflow:hidden;border-radius:${r2};">`
            : `<div style="overflow:hidden;border-radius:${r2};">`;
          const close = it.url ? "</a>" : "</div>";
          return `${wrap}${it.img ? `<img src="${bA(it.img)}" alt="${bA(it.caption || "")}" style="width:100%;height:${h};object-fit:cover;display:block;transition:transform .3s;" />` : `<div style="width:100%;height:${h};background:${bRgb(accent, 0.06)};border:1px dashed ${bRgb(accent, 0.15)};"></div>`}${close}`;
        })
        .join("")}</div>`;
      return `<div style="${ff}${fc}padding:${p};">${inner}</div>`;
    }
    case "imgtext": {
      ...
      const imgHtml = s.img
        ? `<img src="${bA(s.img)}" alt="" style="width:100%;height:${imgH};object-fit:cover;border-radius:${r2};display:block;" />`
        : `<div style="width:100%;height:${imgH};background:${bRgb(accent, 0.06)};border:1px dashed ${bRgb(accent, 0.15)};border-radius:${r2};"></div>`;
      return `<div style="${ff}${fc}padding:${p};"><div style="display:grid;grid-template-columns:${imgW} 1fr;gap:2.5rem;align-items:center;${rev}">\n        <div style="direction:ltr;">${imgHtml}</div>\n        <div style="direction:ltr;">\n          ${s.eyebrow ? `<p style="font-size:.6rem;letter-spacing:.38em;text-transform:uppercase;color:${accent};margin-bottom:.6rem;font-weight:200;">${bE(s.eyebrow)}</p>` : ""}\n          ${s.heading ? `<h2 style="font-size:clamp(1.2rem,3vw,1.8rem);font-weight:300;letter-spacing:.02em;margin-bottom:.8rem;line-height:1.15;">${bE(s.heading)}</h2>` : ""}\n          ${s.body ? `<p style="font-weight:200;font-size:.88rem;line-height:1.85;color:${bRgb(text, 0.68)};">${bE(s.body)}</p>` : ""}\n          ${s.btn_label ? `<a href="${bA(s.btn_url || "#")}" style="display:inline-block;margin-top:1.2rem;font-size:.68rem;font-weight:300;letter-spacing:.2em;text-transform:uppercase;color:${accent};border:1px solid ${bRgb(accent, 0.3)};padding:.55rem 1.4rem;text-decoration:none;">${bE(s.btn_label)}</a>` : ""}\n        </div>\n      </div></div>`;
    }
    case "group": {
      const open = s.default_open !== "no" && s.default_open !== false;
      const inner = (s.sections || []).map((cs) => bRender(cs, theme)).join("");
      return `<details class="bld-group-details" ${open ? "open" : ""}><summary class="bld-group-summary" style="color:${bRgb(text, 0.75)};font-family:'${theme.font}',sans-serif;">${bE(s.label || "Group")}<span style="font-size:.65rem;color:${bRgb(accent, 0.4)};">▾</span></summary><div class="bld-group-body">${inner || `<div class="bld-group-empty">Empty — add sections via editor</div>`}</div></details>`;
    }
    case "accordion": {
      const items = s.items || [];
      const p = s.pad === "sm" ? "1rem 2rem" : s.pad === "lg" ? "4rem 2rem" : "2rem 2rem";
      ...
      const inner = items
        .map(
          (it) =>
            `<details style="border-bottom:1px solid ${bRgb(accent, 0.12)};${bordered ? `border:1px solid ${bRgb(accent, 0.12)};margin-bottom:.5rem;` : ""}">\n        <summary style="cursor:pointer;padding:.85rem ${bordered ? "1rem" : "0"};font-weight:300;font-size:.88rem;letter-spacing:.04em;color:${bRgb(text, 0.88)};list-style:none;display:flex;justify-content:space-between;align-items:center;">${bE(it.q || "Question")}<span style="font-size:.7rem;color:${bRgb(accent, 0.45)};">▼</span></summary>\n        <div class="md-content" style="padding:.5rem ${bordered ? "1rem" : "0"} 1rem;font-size:.82rem;font-weight:200;line-height:1.85;color:${bRgb(text, 0.62)};">${md(it.a)}</div>\n      </details>`
        )
        .join("");
      return `<div style="${ff}${fc}padding:${p};">${inner || `<div style="font-size:.65rem;letter-spacing:.2em;color:${bRgb(accent, 0.25)};text-align:center;padding:1rem;">Add accordion items in editor</div>`}</div>`;
    }
    case "carousel": {
      const items = s.items || [];
      if (!items.length)
        return `<div style="${ff}${fc}padding:2rem;text-align:center;font-size:.65rem;letter-spacing:.2em;color:${bRgb(accent, 0.25)};">Add slides in the editor</div>`;
      ...
      const slides = items
        .map(
          (it, i) =>
            `<div data-slide="${i}" style="display:${i === 0 ? "block" : "none"};">${it.img ? `<img src="${bA(it.img)}" alt="${bA(it.caption || "")}" style="width:100%;height:${h};object-fit:cover;display:block;border-radius:${r2};" />` : `<div style="width:100%;height:${h};background:${bRgb(accent, 0.06)};border:1px dashed ${bRgb(accent, 0.15)};border-radius:${r2};"></div>`}${it.caption ? `<p style="text-align:center;font-size:.62rem;color:${bRgb(text, 0.4)};margin-top:.4rem;font-weight:200;">${bE(it.caption)}</p>` : ""}</div>`
        )
        .join("");
        ...
        s.height === "sm"
          ...
          : s.height === "lg"
            ...
            : s.height === "xl"
              ? "90vh"
              : "520px";
      return `<div style="${ff}${fc}padding:${p};">${s.url ? `<div style="border:1px solid ${bRgb(accent, 0.12)};overflow:hidden;height:${h};display:flex;align-items:center;justify-content:center;font-size:.65rem;letter-spacing:.18em;color:${bRgb(accent, 0.35)};">[ File preview: ${bE(s.url)} ]</div>` : `<div style="border:1px dashed ${bRgb(accent, 0.12)};height:${h};display:flex;align-items:center;justify-content:center;font-size:.58rem;letter-spacing:.22em;text-transform:uppercase;color:${bRgb(accent, 0.2)};">No file selected</div>`}</div>`;
    }
    case "filedown": {
      const p = s.pad === "sm" ? "1.5rem 2rem" : s.pad === "lg" ? "5rem 2rem" : "3rem 2rem";
      const jc = s.align === "left" ? "flex-start" : s.align === "right" ? "flex-end" : "center";
      ...
      const btnCss =
        bs === "outline"
          ...
          : bs === "ghost"
            ? `background:transparent;border:1px solid ${bRgb(text, 0.25)};color:${text};`
            : `background:${accent};border:1px solid ${accent};color:${bg};`;
      return `<div style="${ff}${fc}padding:${p};"><div style="display:flex;flex-direction:column;align-items:${jc};gap:.4rem;"><a href="${bA(s.url || "#")}" style="${btnCss}display:inline-block;font-weight:300;font-size:.75rem;letter-spacing:.22em;text-transform:uppercase;padding:.65rem 1.8rem;text-decoration:none;">↓ ${bE(s.label || "Download")}</a>${s.filename ? `<span style="font-size:.58rem;font-weight:200;letter-spacing:.12em;color:${bRgb(text, 0.38)};">${bE(s.filename)}</span>` : ""}</div></div>`;
    }
    case "tutorials": {
      const p = s.pad === "sm" ? "1rem 2rem" : s.pad === "lg" ? "4rem 2rem" : "2rem 2rem";
      const ha = s.heading_align || "center";
      const headingHtml = s.heading
        ? `<div style="font-size:.82rem;font-weight:200;letter-spacing:.12em;text-transform:uppercase;color:${bRgb(accent, 0.7)};text-align:${ha};margin-bottom:1.2rem;">${bE(s.heading)}</div>`
        : "";
      const maxNote = s.max_items && s.max_items !== "0" ? ` · max ${s.max_items}` : "";
      const styleNote = s.tut_style === "list" ? "List" : "Cards";
      return `<div style="${ff}${fc}padding:${p};">${headingHtml}<div style="border:1px dashed ${bRgb(accent, 0.12)};padding:2rem;text-align:center;font-size:.62rem;letter-spacing:.22em;text-transform:uppercase;color:${bRgb(accent, 0.32)};font-weight:100;">Tutorials — ${styleNote}${maxNote}</div></div>`;
    }
    case "reviews": {
      const p = s.pad === "sm" ? "1rem 2rem" : s.pad === "lg" ? "4rem 2rem" : "2rem 2rem";
      const ha = s.heading_align || "center";
      ...
      return `<div style="${ff}${fc}padding:${p};">${headingHtml}<div style="border:1px dashed ${bRgb(accent, 0.12)};padding:2rem;text-align:center;font-size:.62rem;letter-spacing:.22em;text-transform:uppercase;color:${bRgb(accent, 0.32)};font-weight:100;">Reviews — ${styleNote}${maxNote}</div></div>`;
    }
    default:
      return bXtra(s, {
        E: bE,
...
function bDefault(type) {
  ...
  switch (type) {
    ...
    case "quote":
      return { id: id, type: type, quote: "", attribution: "", variant: "border", pad: "md" };
    case "heading":
      return {
        id: id,
        type: type,
    ...
    case "divider":
      return { id: id, type: type, style: "fade", spacing: "md" };
    case "spacer":
      return { id: id, type: type, size: "md" };
    case "link":
      return {
        id: id,
        type: type,
    ...
    case "group":
      return { id: id, type: type, label: "Group", default_open: "yes", sections: [] };
    case "accordion":
      return {
        id: id,
        type: type,
    ...
    case "fileprev":
      return { id: id, type: type, url: "", height: "md", pad: "md" };
    case "filedown":
      return {
        id: id,
        type: type,
    ...
    case "video":
      return { id: id, type: type, url: "", caption: "", max_w: "normal", pad: "md" };
    case "map":
      return { id: id, type: type, query: "", height: "md", pad: "md" };
    case "marquee":
      return {
        id: id,
        type: type,
    ...
    case "code":
      return { id: id, type: type, code: 'console.log("hello")', lang: "", pad: "md" };
    case "specs":
      return {
        id: id,
        type: type,
    ...
    case "compare":
      return { id: id, type: type, before: "", after: "", height: "md", pad: "md" };
    case "testimonial":
      return {
        id: id,
        type: type,
    ...
    case "qrcode":
      return { id: id, type: type, data: "", caption: "", size: "md", align: "center", pad: "md" };
    case "toggle":
      return {
        id: id,
        type: type,
    ...
    case "copyfield":
      return { id: id, type: type, label: "", value: "hello@example.com", pad: "md" };
    case "embed":
      return { id: id, type: type, url: "", height: "md", pad: "md" };
    case "audio":
      return { id: id, type: type, url: "", pad: "md" };
    case "socialpost":
      return { id: id, type: type, url: "", pad: "md" };
    case "booking":
      return { id: id, type: type, url: "", height: "lg", pad: "md" };
    case "newsletter":
      return {
        id: id,
        type: type,
...
function bEditorFields(s) {
  ...
  if (s.type === "hero") {
    ...
    f = `<div class="bld-ef"><label>Heading</label><input type="text" data-f="heading" value="${bA(s.heading || "")}" /></div>\n       <div class="bld-ef"><label>Show count line</label><select data-f="show_count"><option value="yes"${s.show_count !== "no" ? " selected" : ""}>Yes</option><option value="no"${s.show_count === "no" ? " selected" : ""}>No</option></select></div>\n       <div id="bCOLI">${items
      .map(
        (it, i) =>
          `<div class="bld-li-item"><button class="bld-li-rm" data-crm="${i}">✕</button>\n      ${(() => {
    ...
    const tOpts = Object.entries(typeLabels)
      .filter(([t]) => t !== "group")
      .map(([t, l]) => `<option value="${t}">${l}</option>`)
      .join("");
...
function bBindEditor(panel, s, onUpdate) {
  ...
  panel.querySelectorAll("[data-align]:not([data-align-f])").forEach((btn) => {
    btn.addEventListener("click", () => {
      panel
        .querySelectorAll("[data-align]:not([data-align-f])")
  ...
  panel.querySelectorAll("[data-ci][data-cf]").forEach((inp) => {
    ...
    inp.addEventListener(ev, () => {
      ...
      onUpdate(s, inp.hasAttribute("data-rerender") ? "re-editor" : undefined);
    });
  });
  ...
  panel.addEventListener("contextmenu", (e) => {
    ...
    if (
      !t ||
      (t.tagName !== "TEXTAREA" &&
        !(t.tagName === "INPUT" && /^(text|url|search|email|tel|)$/i.test(t.type || "text")))
    )
      return;
    ...
    const insert = (val) => {
      if (isIcon) {
        ...
        const a = t.selectionStart ?? t.value.length,
          b = t.selectionEnd ?? t.value.length;
  ...
  panel.querySelector("#bFilePrevPick")?.addEventListener("click", () =>
    openFilePicker((url) => {
      s.url = url;
  ...
  panel.querySelector("#bFileDownPick")?.addEventListener("click", () =>
    openFilePicker((url) => {
      s.url = url;
      const inp = panel.querySelector('[data-f="url"]');
      if (inp) {
