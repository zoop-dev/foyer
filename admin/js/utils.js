...
let token = "";
try {
  token = _ls.getItem(KEY) || "";
} catch {
  token = "";
}
if (window.foyerToast) {
  window.foyerToast.configure({ theme: "colored", autoClose: 4e3, position: "top-right" });
}
function classifyToast(msg, isErr) {
  const m = String(msg == null ? "" : msg).toLowerCase();
  if (isErr) {
    if (
      /(required|\bfirst\b|pick a|pick or|select |type a|no page|not supported|coming soon|isn'?t configured|not configured|too large|denied|permission|max )/.test(
        m
      )
    )
      return "warning";
  ...
  if (
    /(discard|no unsaved|no changes|copied|moved to|sub-page|top level|pushed|fixed format|builds block|added to nav|removed from nav|skipped|^restored unsaved)/.test(
      m
    )
  )
    return "info";
...
function toast(msg, isErr, opts) {
  if (window.foyerToast) {
    ...
    return window.foyerToast(msg, Object.assign({ type: type }, opts));
  }
...
const dlg = (() => {
  function show({ title: title, message: message, buttons: buttons }) {
    return new Promise((resolve) => {
      ...
      buttons.forEach(({ label: label, value: value, style: style }) => {
...
function mainSiteSessionToken() {
  try {
    return (JSON.parse(_ls.getItem("foyer_session") || "null") || {}).session_token || "";
  } catch {
    return "";
  }
}
function timeAgo(str) {
  return F.fmt.timeAgo(str);
}
function escHtml(s) {
  return F.esc(s);
}
...
function pickImageSize() {
  return new Promise((resolve) => {
    ...
    bd.style.cssText =
      "position:fixed;inset:0;z-index:99991;background:rgba(0,0,0,.65);display:flex;align-items:center;justify-content:center;padding:1.5rem;";
    ...
    bd.querySelectorAll("[data-w]").forEach((b) =>
      b.addEventListener("click", () => {
        bd.remove();
        resolve(b.dataset.w || null);
      })
    );
...
function showUpdateOverlay(pendingUiVersion, newVer) {
  ...
  _updateShown = true;
  const cacheNewVersion = () => {
    if (pendingUiVersion) _ls.setItem(UI_VERSION_KEY, pendingUiVersion);
  };
  const isFoyer = !pendingUiVersion;
  const knownVer = isFoyer ? VERSION : _ls.getItem(UI_VERSION_KEY) || VERSION;
  const _fIcon = (w, h, style) =>
    `<svg viewBox="0 0 44 50" width="${w}" height="${h}" fill="none" stroke="rgba(var(--accent-rgb),.85)" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;${style || ""}"><path d="M5 46 V24 a16 16 0 0 1 32 0 V46"/><path d="M15 46 V28 a6 6 0 0 1 12 0 V46"/></svg>`;
  ...
  setTimeout(() => {
    ...
    el.id = "_ver_overlay";
    el.style.cssText =
      'position:fixed;inset:0;z-index:99999;background:rgba(var(--bg-rgb),.0);display:flex;align-items:center;justify-content:center;font-family:"Josefin Sans",sans-serif;transition:background .4s ease;';
    ...
    el.querySelector("#_verReloadBtn").addEventListener("mouseover", function () {
      this.style.background = "rgba(var(--accent-rgb),.1)";
    });
    el.querySelector("#_verReloadBtn").addEventListener("mouseout", function () {
...
(function startVersionPoll() {
  ...
  const id = setInterval(async () => {
    ...
    if (sys && sys !== VERSION) {
      let tried = null;
      try {
        tried = sessionStorage.getItem("foyer_sys_reloaded");
      } catch {}
      if (tried !== sys) {
        try {
          sessionStorage.setItem("foyer_sys_reloaded", sys);
        } catch {}
        showUpdateOverlay(undefined, sys);
        return;
      }
    }
    ...
    if (data.ui_version && storedUi && data.ui_version !== storedUi) {
      showUpdateOverlay(data.ui_version, data.ui_version);
      return;
    }
  }, 1e4);
})();
...
function enterRateLimitMode() {
  ...
  if (!document.getElementById("rl-screen")) {
    ...
    el.id = "rl-screen";
    el.style.cssText =
      'position:fixed;inset:0;z-index:9998;background:var(--bg);display:flex;align-items:center;justify-content:center;flex-direction:column;gap:1.2rem;font-family:"Josefin Sans",sans-serif;';
...
async function pushUiUpdate() {
  ...
  const cur = await fetch("/api/version")
    .then((r) => r.json())
    .catch(() => null);
  ...
  if (res.ok) {
    _ls.setItem(UI_VERSION_KEY, newVer);
    if (btn) {
...
async function enableLoginMagic() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const data = await fetch("/api/auth/magic-link", {
      ...
      body: JSON.stringify({ email: email, redirect: "admin" }),
    })
      .then((r) => r.json())
      .catch(() => null);
    if (data?.ok) {
      ...
      sent.textContent =
        "Check your inbox — open the link on any device. This page will sign in automatically.";
...
function pollLoginMagic(requestId, sentEl) {
  const id = setInterval(async () => {
    ...
    if (s.status === "approved") {
      ...
      const r = await fetch("/api/admin-check", { headers: { "X-Session-Token": s.session_token } })
        .then((x) => x.json())
        .catch(() => null);
      ...
      clearInterval(id);
      if (sentEl)
        sentEl.textContent = "That link expired before it was used. Refresh to try again.";
    }
  }, 2500);
}
...
async function boot() {
  ...
  if (loginScreen) {
    ...
    setTimeout(() => loginScreen.remove(), 450);
  }
  app.classList.add("visible");
  _adminActive = true;
  if (typeof foyerTermsGate === "function") await foyerTermsGate();
  await fetchSettings();
  await fetchImages();
  renderImgTabGallery();
  bldBoot();
...
function showReauthScreen() {
  ...
  el.id = "reauth-screen";
  el.style.cssText =
    'position:fixed;inset:0;z-index:99996;background:rgba(var(--bg-rgb),.97);display:flex;align-items:center;justify-content:center;flex-direction:column;gap:1.3rem;font-family:"Josefin Sans",sans-serif;padding:2rem;';
  ...
  el.querySelector("#reauthBtn").addEventListener("click", async () => {
    ...
    if (st) {
      const r = await _origFetch("/api/admin-check", { headers: { "X-Session-Token": st } })
        .then((x) => x.json())
        .catch(() => null);
      if (r && (r.role === "admin" || r.role === "owner")) {
        sessionToken = st;
...
document.getElementById("oauthSignInBtn")?.addEventListener("click", async () => {
  ...
  const st = mainSiteSessionToken();
  if (st) {
    const r = await fetch("/api/admin-check", { headers: { "X-Session-Token": st } })
      .then((x) => x.json())
      .catch(() => null);
    if (r && (r.role === "admin" || r.role === "owner")) {
      sessionToken = st;
...
async function adminLogout() {
  ...
  _ls.removeItem(KEY);
  if (sessionToken) _ls.removeItem("foyer_session");
  token = "";
  sessionToken = "";
  location.reload();
}
  ...
  async function chk() {
    try {
      ...
      if (r.ok) {
        ...
        if (s && (s.offline === true || s.licensed === false)) location.replace("/offline");
      }
    } catch {}
  }
  chk();
