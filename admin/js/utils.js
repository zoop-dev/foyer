import { foyerTermsGate } from "./terms-gate.js";
...
function classifyToast(msg, isErr) {
  ...
  if (isErr) {
    if (/(required|\bfirst\b|pick a|pick or|select |type a|no page|not supported|coming soon|isn'?t configured|not configured|too large|denied|permission|max )/.test(
      m
    ))
      return "warning";
    return "error";
  }
  if (/(discard|no unsaved|no changes|copied|moved to|sub-page|top level|pushed|fixed format|builds block|added to nav|removed from nav|skipped|^restored unsaved)/.test(
    m
  ))
    return "info";
  return "success";
}
export function toast(msg, isErr, opts) {
  if (window.foyerToast) {
    opts = opts || {};
...
export function wireMdToolbar(scope, ta, onChange) {
  scope.querySelectorAll(".md-toolbar [data-md]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (a === "bold") mdWrap(ta, "**", "**", "bold text");
        ...
        import("./main.js").then(({ openImgPicker }) => {
...
function pickImageSize() {
  return new Promise((resolve) => {
    ...
    bd.querySelectorAll("[data-w]").forEach(
...
export async function pushUiUpdate() {
...
async function boot() {
  ...
  const { fetchSettings, fetchImages, renderImgTabGallery } = await import("./main.js");
  await fetchSettings();
  await fetchImages();
  renderImgTabGallery();
  bldBoot();
...
window.fetch = async (...args) => {
  ...
  try {
    ...
    if (_adminActive && res.status === 401 && url.includes("/api/") && !url.includes("/api/auth") && !url.includes("/api/admin-check")) {
      showReauthScreen();
    }
  } catch {
  }
