import { authHeaders, dlg, escAttr, escHtml, timeAgo, toast } from "./utils.js";
...
export async function fetchAnalytics() {
  ...
  const roleBadge = (r) => r === "owner" ? '<span class="v-role-badge owner">OWNER</span>' : r === "admin" ? '<span class="v-role-badge admin">ADMIN</span>' : "";
  vList.innerHTML = _rolesBtn + d.visitors.map(
    (v, i) => `
...
async function openRolesModal() {
  ...
  async function refresh() {
    ...
    body.querySelectorAll("[data-redit]").forEach(
      (b) => b.addEventListener("click", () => {
        editId = +b.dataset.redit;
        refresh();
      })
    );
    body.querySelectorAll("[data-rdel]").forEach(
      (b) => b.addEventListener("click", async () => {
        if (!await dlg.confirm("Delete this role? Admins using it revert to full access.", {
          danger: true,
          confirm: "Delete"
        }))
          return;
    ...
    body.querySelectorAll("[data-assign]").forEach(
...
async function changeRole(id, action) {
  ...
  const label = action === "promote" ? `Promote ${v?.name || v?.email || "this user"} to admin? They'll be able to access the admin panel.` : `Demote ${v?.name || v?.email || "this admin"} back to a regular user?`;
  if (!await dlg.confirm(label, {
...
async function banVisitor(id, banning) {
  if (!banning) {
    if (!await dlg.confirm(
      ...
      return;
    const res2 = await fetch(`/api/visitors/${id}/unban`, {
      method: "POST",
      headers: authHeaders()
    });
    if (!res2.ok) {
...
function openVisitorModal(v) {
  ...
  backdrop.id = "vModal";
  const avatarHtml = v.picture ? `<img class="v-modal-head-avatar" src="${escHtml(v.picture)}" referrerpolicy="no-referrer" />` : `<div class="v-modal-head-ph">${escHtml((v.name || v.email || "?").charAt(0).toUpperCase())}</div>`;
  backdrop.innerHTML = `
    <div class="v-modal">
  ...
  fetch(`/api/analytics/visitor/${v.id}`, { headers: authHeaders() }).then((r) => r.json()).then((d) => {
