function foyerTermsGate() {
  return new Promise(async (resolve) => {
    let status = null;
    try {
      status = await fetch("/api/terms/status", { headers: authHeaders() }).then((r) =>
        r.ok ? r.json() : null
      );
    } catch (e) {}
    ...
    ov.style.cssText =
      'position:fixed;inset:0;z-index:100002;background:rgba(8,11,9,.86);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:1.5rem;font-family:"Josefin Sans",system-ui,sans-serif;';
    ...
    ov.querySelector("#foyerTermsDecline").addEventListener("click", () => {
      location.href = "/";
    });
  });
}
