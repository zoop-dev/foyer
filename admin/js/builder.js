function bSetupDnD(canvasEl, sections, onDone) {
  canvasEl.querySelectorAll(".bld-sw").forEach((sw) => {
    ...
    sw.addEventListener("dragend", () => {
      ...
      canvasEl
        .querySelectorAll(".bld-sw")
        .forEach((s) => s.classList.remove("drag-above", "drag-below"));
      bDragId = null;
    });
    sw.addEventListener("dragover", (e) => {
      ...
      canvasEl
        .querySelectorAll(".bld-sw")
        .forEach((s) => s.classList.remove("drag-above", "drag-below", "drag-left", "drag-right"));
      if (sideDrop) {
...
function bldRenderPicker() {
  ...
  if (bldPickerCat === "__saved") {
    ...
    grid.innerHTML = rows.length
      ? rows
          .map(
            (s) =>
              `<button class="bld-pick" data-saved="${s.id}" title="${bA(s.label)}" style="position:relative;"><span class="bld-pick-ic">✦</span><span class="bld-pick-l">${bE(s.label)}</span><span data-savdel="${s.id}" title="Remove" style="position:absolute;top:1px;right:4px;font-size:1rem;line-height:1;opacity:.5;">×</span></button>`
          )
          .join("")
      : `<p class="bld-picker-empty">No saved blocks yet. Open any block’s menu → <b>Save as reusable</b>.</p>`;
    grid.querySelectorAll(".bld-pick").forEach((b) =>
      b.addEventListener("click", (e) => {
        if (e.target.closest("[data-savdel]")) return;
        bldInsertSaved(+b.dataset.saved);
      })
    );
    grid.querySelectorAll("[data-savdel]").forEach((b) =>
      b.addEventListener("click", (e) => {
        e.stopPropagation();
  ...
  if (bldPickerQ)
    list = list.filter((b) =>
      (b.l + " " + b.t + " " + (b.k || "")).toLowerCase().includes(bldPickerQ)
    );
  if (!list.length) {
  ...
  grid.innerHTML = html;
  grid
    .querySelectorAll(".bld-pick")
    .forEach((b) => b.addEventListener("click", () => bldAddBlock(b.dataset.type)));
}
function bldBuildPicker() {
  ...
  cats.innerHTML =
    `<button class="bld-picker-cat on" data-cat="all">All blocks</button>` +
    (window.foyerPlan === "ultra"
      ? `<button class="bld-picker-cat" data-cat="__saved">✦ Saved</button>`
      : "") +
    BLK_CATS.map(
  ...
  cats.querySelectorAll(".bld-picker-cat").forEach((b) =>
...
function bldOpenPicker() {
  ...
  if (s) s.value = "";
  document
    .getElementById("bldPickerCats")
    .querySelectorAll(".bld-picker-cat")
    .forEach((x) => x.classList.toggle("on", x.dataset.cat === "all"));
  bldRenderPicker();
...
function ctxMenu(x, y, items) {
  ...
  m.innerHTML = items
    .map((it, i) =>
      it === "-"
        ? '<div class="bld-ctx-sep"></div>'
        : `<button class="bld-ctx-item${it.danger ? " danger" : ""}" data-i="${i}"><span class="bld-ctx-ico">${it.icon || ""}</span>${escHtml(it.label)}</button>`
    )
    .join("");
  ...
  m.querySelectorAll(".bld-ctx-item").forEach((b) =>
    b.addEventListener("click", () => {
...
async function bldOpenIconPicker(cb, anchor) {
  ...
  pop.id = "bldEmojiPop";
  pop.style.cssText =
    "position:fixed;z-index:99993;width:" +
    W +
    "px;border-radius:10px;overflow:hidden;box-shadow:0 12px 40px rgba(0,0,0,.5);background:var(--panel);border:1px solid rgba(77,189,106,.2);";
  const place = () => {
  ...
  try {
    ...
    host.innerHTML =
      '<div style="padding:1rem 1.2rem;color:var(--muted);font-size:.75rem;">Couldn’t load the emoji picker.</div>';
  }
...
function bldBlockMenuItems(id) {
  ...
  return [
    ...
    ...(typeof foyerInteractive === "function" &&
    foyerInteractive(bldState.sections[i]?.type) &&
    foyerInteractionsBeta()
      ? [{ label: "Interactions (beta)", icon: ico("bolt"), action: () => openInteractions(id) }]
      : []),
    ...(bldAiOn
      ? [{ label: "Polish copy with AI", icon: ico("sparkles"), action: () => bldPolishBlock(id) }]
      : []),
    { label: "Duplicate", icon: ico("copy"), action: () => bldDuplicateBlock(id) },
    ...(window.foyerPlan === "ultra"
      ? [{ label: "Save as reusable", icon: ico("copy"), action: () => bldSaveReusable(id) }]
      : []),
...
function bldAiSchema() {
  return BLOCK_CATALOG.filter((b) => !AI_BLOCK_DENY.has(b.t))
    .map((b) => {
      ...
      const fields = Object.keys(def)
        .filter((k) => k !== "id" && k !== "type" && k !== "access_key")
        .map((k) => hint(k, def[k]))
        .join(", ");
...
function bldApplyAi(arr) {
  ...
  let n = 0;
  arr
    .filter((b) => b && b._op === "remove" && Number.isInteger(b._at))
...
async function bldPolishBlock(id) {
  ...
  try {
    ...
    let op =
      arr.find((o) => o && o._op === "update" && o._at === idx) ||
      arr.find((o) => o && o._op === "update");
    if (!op && arr.length === 1 && arr[0] && arr[0].type === sec.type && !arr[0]._op) op = arr[0];
    if (op) {
      const { _op: _op, _at: _at, ...rest } = op;
...
const _foyerMark =
  '<svg viewBox="0 0 44 50" width="15" height="17" fill="none" stroke="var(--green)" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 46 V24 a16 16 0 0 1 32 0 V46"/><path d="M15 46 V28 a6 6 0 0 1 12 0 V46"/></svg>';
function bldAssistant() {
  ...
  function render() {
    if (!bldAiChat.length) {
      ...
      const chips = editing
        ? ["Add a pricing section", "Add an FAQ", "Make the copy punchier"]
        : ["A coffee-shop landing page", "A photography portfolio", "A résumé site"];
    ...
    msgsEl.innerHTML = bldAiChat
      .map(
        (m) =>
          `<div class="bld-ai-msg ${m.role}">${m.role === "assistant" ? `<span class="bld-ai-av">${_foyerMark}</span>` : ""}<div class="bld-ai-bubble">${m.thinking ? '<span class="bld-ai-dots"><i></i><i></i><i></i></span>' : escHtml(m.content)}${m.applied ? `<div class="bld-ai-applied">✓ Updated the page · ${m.applied} section${m.applied === 1 ? "" : "s"}</div>` : ""}</div></div>`
      )
      .join("");
...
async function bldLoadSaved() {
  try {
    bldSavedBlocks = await fetch("/api/saved-blocks", { headers: authHeaders() }).then((r) =>
      r.ok ? r.json() : []
    );
  } catch {
    bldSavedBlocks = [];
...
async function bldSaveReusable(id) {
  ...
  const r = await fetch("/api/saved-blocks", {
    method: "POST",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify({ label: label, json: clean }),
  });
  if (r.ok) {
    bldSavedBlocks = null;
...
function bldSetAutosave(mode) {
  const wrap = document.getElementById("bldAutosave"),
    txt = document.getElementById("bldAutosaveText");
  if (!wrap || !txt) return;
...
function bldDrawPages() {
  ...
  const bulkBar = ultra
    ? `<div id="bldBulkBar" style="display:none;align-items:center;gap:.5rem;padding:.4rem .6rem;background:rgba(var(--accent-rgb),.1);border-bottom:1px solid var(--border);font-size:.7rem;color:var(--white);"><span><b id="bldBulkN">0</b> selected</span><button class="btn btn-xs" id="bldBulkTrash" style="color:#e0608a;">Move to Trash</button><button class="btn btn-xs" id="bldBulkClear">Clear</button></div>`
    : "";
  list.innerHTML =
    bulkBar +
    bldPages
      ...
      .join("") +
    (ultra
      ? `<button id="bldTrashBtn" type="button" style="display:flex;align-items:center;gap:.45rem;width:100%;background:none;border:none;border-top:1px solid var(--border);color:var(--muted);font:inherit;font-size:.72rem;padding:.6rem .7rem;cursor:pointer;margin-top:.2rem;">🗑 Trash</button>`
      : "");
  list.querySelector("#bldTrashBtn")?.addEventListener("click", (e) => {
    e.stopPropagation();
    bldOpenTrash();
  });
  const bar = list.querySelector("#bldBulkBar"),
    nEl = list.querySelector("#bldBulkN");
  ...
  list.querySelectorAll(".bld-pi-chk").forEach((c) =>
  ...
  list.querySelector("#bldBulkTrash")?.addEventListener("click", async (e) => {
    ...
    if (
      !(await dlg.confirm(`Move ${ids.length} page${ids.length > 1 ? "s" : ""} to Trash?`, {
        danger: true,
    ...
    await Promise.all(
      ids.map((id) =>
        fetch(`/api/pages/${id}`, { method: "DELETE", headers: authHeaders() }).catch(() => {})
      )
    );
  ...
  list.querySelectorAll(".bld-pi").forEach((el) =>
    el.addEventListener("click", (e) => {
      if (
        e.target.closest(".bld-pi-del") ||
        e.target.closest("[data-discard]") ||
        e.target.closest(".bld-pi-chk")
      )
        return;
      bldPickPage(+el.dataset.pid);
    })
  );
  list.querySelectorAll("[data-discard]").forEach((badge) =>
...
async function bldOpenTrash() {
  let rows = [];
  try {
    rows = await fetch("/api/pages/trash", { headers: authHeaders() }).then((r) =>
      r.ok ? r.json() : []
    );
  } catch (e) {}
  ...
  ov.querySelector("#bldTrashRestoreAll")?.addEventListener("click", async () => {
    await Promise.all(
      rows.map((p) =>
        fetch(`/api/pages/${p.id}/restore`, { method: "POST", headers: authHeaders() }).catch(
          () => {}
        )
      )
    );
  ...
  ov.querySelectorAll("[data-trestore]").forEach((b) =>
  ...
  ov.querySelectorAll("[data-tpurge]").forEach((b) =>
    b.addEventListener("click", async () => {
      const id = +b.dataset.tpurge;
      if (
        !(await dlg.confirm("Permanently delete this page? This cannot be undone.", {
          danger: true,
...
function bSecWrap(s, inner) {
  ...
  const badge = s.hide
    ? `<div style="position:absolute;top:.4rem;left:.4rem;z-index:2;font-size:.5rem;letter-spacing:.12em;text-transform:uppercase;background:rgba(230,200,90,.9);color:#1a1400;padding:.12rem .4rem;border-radius:3px;">Hidden · ${s.hide}</div>`
    : "";
  return ws || badge
    ? `<div style="position:relative;${ws}${s.hide ? "opacity:.5;" : ""}">${badge}${inner}</div>`
    : inner;
}
...
function bldWireTextEditor(root) {
  ...
  root.querySelector("#txtCoverPick")?.addEventListener("click", () =>
    openImgPicker((url) => {
      bldState.cover = url;
...
function bldDrawCanvas() {
  ...
  el.querySelectorAll("[data-polish]").forEach((b) =>
    b.addEventListener("click", (e) => {
      e.stopPropagation();
      bldPolishBlock(b.dataset.polish);
    })
  );
  el.querySelectorAll("[data-interact]").forEach((b) =>
    b.addEventListener("click", (e) => {
      e.stopPropagation();
  ...
  el.querySelectorAll("[data-del]").forEach((b) =>
    b.addEventListener("click", (e) => {
      ...
      const id = b.dataset.del,
        sec = bldState.sections.find((x) => x.id === id);
  ...
  el.querySelectorAll("[data-wtog]").forEach((b) =>
    b.addEventListener("click", (e) => {
      e.stopPropagation();
  ...
  el.querySelectorAll("[data-gc-edit]").forEach((b) =>
    b.addEventListener("click", (e) => {
      e.stopPropagation();
  ...
  el.querySelectorAll("[data-gc-del]").forEach((b) =>
    b.addEventListener("click", (e) => {
      e.stopPropagation();
...
async function bldSavePageMeta(id, name, slug) {
  ...
  const r = await fetch(`/api/pages/${id}`, {
    method: "PUT",
    headers: { ...authHeaders(), "Content-Type": "application/json" },
    body: JSON.stringify({ title: name, slug: slug }),
  });
  if (!r.ok) {
    const d = await r.json().catch(() => ({}));
...
function bldDrawEditor() {
  ...
  if (!bldSel) {
    if (bldPageId) {
      ...
      panel.querySelector("#pgImagePick").addEventListener("click", () =>
        openImgPicker((url) => {
      ...
      panel
        .querySelector("#pgPwClear")
        ?.addEventListener("click", () => bldSavePagePassword(bldPageId, ""));
    } else {
  ...
  if (!s) return;
  const { html: html, label: label } = bEditorFields(s);
  const backBtn = parentGroup
    ? `<button class="btn btn-xs" id="bEdBack" style="margin-bottom:.6rem;width:100%;">← Back to group</button>`
    : "";
  const interactBtn =
    !parentGroup &&
    typeof foyerInteractive === "function" &&
    foyerInteractive(s.type) &&
    typeof foyerInteractionsBeta === "function" &&
    foyerInteractionsBeta()
...
async function bldBoot() {
  if (bldBooted) return;
  bldBooted = true;
  fetch("/api/ai/page", { headers: authHeaders() })
    .then((r) => r.json())
    .then((d) => {
  ...
  const _fab = document.getElementById("bldAddFab"),
    _picker = document.getElementById("bldOpenPicker");
  ...
  document.getElementById("bldNPCreate").addEventListener("click", async () => {
    ...
    const res = await fetch("/api/pages", {
      method: "POST",
      headers: { ...authHeaders(), "Content-Type": "application/json" },
      body: JSON.stringify({ title: title, slug: slug, page_json: initJson }),
    });
    if (!res.ok) {
    ...
    bldPages.push({ id: data.id, title: title, slug: slug, page_json: initJson, is_published: 1 });
    bldDrawPages();
    bldPickPage(data.id);
  });
  ...
  document.getElementById("bldPublish").addEventListener("click", async () => {
    ...
    const sp = document.getElementById("bldSpinner"),
      btn = document.getElementById("bldPublish");
...
async function bldLoadPages() {
  ...
  bldDrawPages();
  const toSelect =
    prevId && bldPages.find((p) => p.id === prevId) ? prevId : bldPages[0]?.id || null;
