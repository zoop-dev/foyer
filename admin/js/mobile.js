...
function mSyncTitle() {
  ...
  if (!el) return;
  if (mCurTab === "builder") {
    const p = typeof bldPages !== "undefined" ? bldPages.find((p2) => p2.id === bldPageId) : null;
...
function mBuildMenu() {
  ...
  list.innerHTML = visible.map(
    (s) => `<button class="m-menu-item${s.tab === mCurTab ? " on" : ""}" data-tab="${s.tab}"><span class="m-menu-ico">${mIco(s.ico)}</span>${s.label}${s.tab === "analytics" ? '<span class="m-menu-badge" id="mMenuBadge" style="display:none"></span>' : ""}</button>`
  ).join("");
  list.querySelectorAll("[data-tab]").forEach(
...
function mRenderBlockList() {
  ...
  list.innerHTML = grouped.map(
    (r) => r.length === 1 ? mBlockRow(r[0]) : `<div class="m-blist-row">${r.map(mBlockRow).join("")}</div>`
  ).join("");
  list.querySelectorAll(".m-block").forEach((row) => {
    ...
    ["pointermove", "pointerup", "pointercancel"].forEach(
      (ev) => prev.addEventListener(ev, () => {
...
function mOpenPages() {
  ...
  const rows = (bldPages || []).map(
    (p) => `<div class="m-page-row${p.id === bldPageId ? " on" : ""}" data-pid="${p.id}"><div style="flex:1;min-width:0;"><div class="m-page-row-t">${escHtml(p.title || "Untitled")}${unsaved(p) ? ' <span style="color:var(--accent);font-size:.7rem;">• unsaved</span>' : ""}</div><div class="m-page-row-s">${escHtml(p.slug || "")}</div></div>${unsaved(p) ? `<button class="btn btn-xs btn-danger" data-disc="${p.id}" style="flex-shrink:0;">Discard</button>` : ""}<button class="btn btn-xs" data-edit="${p.id}" style="flex-shrink:0;">Rename</button></div>`
  ).join("") || '<p style="color:var(--muted);font-size:.8rem;padding:1rem 0;">No pages yet.</p>';
  mSheetMode = "pages";
  mOpenSheet(
    "Pages",
    rows + `<button class="btn btn-primary" style="width:100%;margin-top:.7rem" id="mNewPage">+ New page</button>`,
    true
  );
  document.querySelectorAll("#mSheetBody [data-pid]").forEach(
  ...
  document.querySelectorAll("#mSheetBody [data-edit]").forEach(
    (b) => b.addEventListener("click", (e) => {
      e.stopPropagation();
      mEditPageForm(+b.dataset.edit);
    })
  );
  document.querySelectorAll("#mSheetBody [data-disc]").forEach(
...
function mUpdatePageChip() {
  ...
  if (!el) return;
  const p = (bldPages || []).find((p2) => p2.id === bldPageId);
