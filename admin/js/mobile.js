...
const M_SECTIONS = [
  ...
  { tab: "history", label: "History", ico: "clock" },
];
const mIco = (name) =>
  `<span class="m-ico-svg" style="-webkit-mask:url('/assets/icons/${name}.svg') center/contain no-repeat;mask:url('/assets/icons/${name}.svg') center/contain no-repeat;"></span>`;
let mCurTab = "builder";
let mSheetMode = "";
function mSyncTitle() {
...
function mSetActions() {
  const el = document.getElementById("mActions");
  if (!el) return;
  if (mCurTab === "builder") {
...
function mBuildMenu() {
  const list = document.getElementById("mMenuList");
  if (!list) return;
  ...
  list.innerHTML = visible
    .map(
      (s) =>
        `<button class="m-menu-item${s.tab === mCurTab ? " on" : ""}" data-tab="${s.tab}"><span class="m-menu-ico">${mIco(s.ico)}</span>${s.label}${s.tab === "analytics" ? '<span class="m-menu-badge" id="mMenuBadge" style="display:none"></span>' : ""}</button>`
    )
    .join("");
  list.querySelectorAll("[data-tab]").forEach((b) =>
    b.addEventListener("click", () => {
      mSwitch(b.dataset.tab);
      mCloseMenu();
    })
  );
  mSyncMenuBadge();
...
function mSyncMenuBadge() {
  const badge = document.getElementById("mMenuBadge");
  const vis = document.getElementById("visBadge");
  if (badge && vis) {
    badge.textContent = vis.textContent;
    badge.style.display = (vis.textContent || "").trim() ? "" : "none";
  }
}
...
function mOpenSheet(title, html, compact) {
  const sheet = document.getElementById("mSheet"),
    body = document.getElementById("mSheetBody");
  mRescueTheme();
...
function mBlockRow(s) {
  const cat =
    typeof BLOCK_CATALOG !== "undefined" ? BLOCK_CATALOG.find((b) => b.t === s.type) : null;
  ...
  if (s.type === "group") {
    ...
    preview = bSecWrap(
      s,
      bRender(s, bldState) || `<div class="m-block-empty">Empty — tap Edit to fill it in</div>`
    );
  }
...
function mRenderBlockList() {
  if (!isMobile()) return;
  const col = document.querySelector("#sec-builder .bld-canvas-col");
  if (!col) return;
  let list = document.getElementById("mBldList");
  if (!list) {
    list = document.createElement("div");
    list.id = "mBldList";
    list.className = "m-blist";
    col.appendChild(list);
  }
  const secs = (typeof bldState !== "undefined" && bldState.sections) || [];
  if (!bldPageId) {
    list.innerHTML = `<div class="m-blist-empty">Pick or create a page to start.<br>Tap the page name above.</div>`;
    return;
  }
  if (bldState.kind === "text" && typeof bldTextEditorHtml === "function") {
    list.innerHTML = bldTextEditorHtml();
    bldWireTextEditor(list);
    return;
  }
  if (!secs.length) {
    list.innerHTML = `<div class="m-blist-empty">No sections yet.<br>Tap <b>＋ Section</b> to start building.</div>`;
    return;
  }
  ...
  list.innerHTML = grouped
    .map((r) =>
      r.length === 1
        ? mBlockRow(r[0])
        : `<div class="m-blist-row">${r.map(mBlockRow).join("")}</div>`
    )
    .join("");
  list.querySelectorAll(".m-block").forEach((row) => {
    ...
    row.querySelector(".m-block-edit").addEventListener("click", (e) => {
      e.stopPropagation();
      bldSel = id;
      bldParentId = null;
      mOpenEditor(id);
    ...
    prev.addEventListener("pointerdown", (e) => {
      ...
      lpTimer = setTimeout(() => {
        ...
        ctxMenu(e.clientX, e.clientY, bldBlockMenuItems(id));
      }, 480);
    });
    ["pointermove", "pointerup", "pointercancel"].forEach((ev) =>
      prev.addEventListener(ev, () => {
        clearTimeout(lpTimer);
      })
    );
    row.querySelector(".m-block-del").addEventListener("click", (e) => {
      e.stopPropagation();
      mDeleteBlock(id);
    });
    row.querySelector(".m-block-w").addEventListener("click", (e) => {
      e.stopPropagation();
      mToggleWidth(id);
    });
    mWireDrag(row);
...
function mToggleWidth(id) {
  const s = bldState.sections.find((x) => x.id === id);
  if (!s) return;
...
function mOpenEditor(id) {
  const found = bldFindSection(id);
  if (!found || !found.sec) return;
  const sec = found.sec;
  const { html: html, label: label } = bEditorFields(sec);
  mSheetMode = "editor";
  ...
  bBindEditor(document.getElementById("mSheetBody"), sec, (s, mode) => {
    ...
    if (mode === "re-editor") mOpenEditor(id);
  });
...
function mOpenPages() {
  ...
  const rows =
    (bldPages || [])
      .map(
        (p) =>
          `<div class="m-page-row${p.id === bldPageId ? " on" : ""}" data-pid="${p.id}"><div style="flex:1;min-width:0;"><div class="m-page-row-t">${escHtml(p.title || "Untitled")}${unsaved(p) ? ' <span style="color:var(--accent);font-size:.7rem;">• unsaved</span>' : ""}</div><div class="m-page-row-s">${escHtml(p.slug || "")}</div></div>${unsaved(p) ? `<button class="btn btn-xs btn-danger" data-disc="${p.id}" style="flex-shrink:0;">Discard</button>` : ""}<button class="btn btn-xs" data-edit="${p.id}" style="flex-shrink:0;">Rename</button></div>`
      )
      .join("") ||
    '<p style="color:var(--muted);font-size:.8rem;padding:1rem 0;">No pages yet.</p>';
  mSheetMode = "pages";
  mOpenSheet(
    "Pages",
    rows +
      `<button class="btn btn-primary" style="width:100%;margin-top:.7rem" id="mNewPage">+ New page</button>`,
    true
  );
  document.querySelectorAll("#mSheetBody [data-pid]").forEach((r) =>
    r.addEventListener("click", (e) => {
      if (e.target.closest("[data-edit]") || e.target.closest("[data-disc]")) return;
      bldPickPage(+r.dataset.pid);
      mCloseSheet();
      mSyncTitle();
      mUpdatePageChip();
      mRenderBlockList();
  ...
  document.querySelectorAll("#mSheetBody [data-edit]").forEach((b) =>
    b.addEventListener("click", (e) => {
      e.stopPropagation();
      mEditPageForm(+b.dataset.edit);
    })
  );
  document.querySelectorAll("#mSheetBody [data-disc]").forEach((b) =>
    b.addEventListener("click", async (e) => {
      e.stopPropagation();
      await bldDiscardDraft(+b.dataset.disc);
      mOpenPages();
    })
  );
...
function mUpdatePageChip() {
  const el = document.getElementById("mBldPageName");
  if (!el) return;
...
bldDrawEditor = function () {
  ...
  if (bldSel) {
    const f = bldFindSection(bldSel);
    if (f && f.sec) mOpenEditor(bldSel);
  }
};
  ...
  const sync = () => {
    ...
    if (editing && !editor.querySelector(".m-edit-back")) {
      ...
      back.addEventListener("click", () =>
        document.getElementById(secId).classList.remove("m-editing")
      );
      editor.prepend(back);
    }
  };
...
(function wireMobileShell() {
  ...
  on("mBldAi", "click", () => {
    if (typeof bldAssistant === "function") bldAssistant();
  });
  on("mBldTheme", "click", mOpenTheme);
  on("mBldAdd", "click", () => {
    if (typeof bldOpenPicker === "function") bldOpenPicker();
  });
  const vis = document.getElementById("visBadge");
  if (vis)
    new MutationObserver(mSyncMenuBadge).observe(vis, {
      childList: true,
      characterData: true,
      subtree: true,
      attributes: true,
