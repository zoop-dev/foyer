...
const PERM_RULES = [
  {
    ...
    test: (route, write) =>
      write &&
      (/^pages(\/|$)/.test(route) || /^saved-blocks(\/|$)/.test(route) || route === "nav/order"),
  },
  ...
  {
    need: "comments",
    test: (route, _write, method) =>
      route === "comments/admin" || (/^comments\/\d+$/.test(route) && method === "DELETE"),
  },
...
export async function onRequest(context) {
  const { request: request, env: env, params: params } = context;
  ...
  for (const { handler: handler } of HANDLERS) {
    const res = await handler(ctx);
    if (res) {
      if (
        ctx._adminRole &&
        (_m === "POST" || _m === "PUT" || _m === "DELETE") &&
        res.status >= 200 &&
        res.status < 300
      ) {
        bumpEpoch(ctx);
