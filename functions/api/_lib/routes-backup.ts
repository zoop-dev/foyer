...
async function backupUsage(env: Env, host: string): Promise<{ quota: number; used: number }> {
  ...
  try {
    const r = await fetch(
      `${base}/rest/v1/foyer_sites?domain=eq.${encodeURIComponent(host)}&select=backup_quota,plan`,
      { headers: H }
    );
    if (r.ok) {
  ...
  try {
    const r = await fetch(
      `${base}/rest/v1/foyer_backups?domain=eq.${encodeURIComponent(host)}&select=id`,
...
async function recordBackup(env: Env, host: string, scope: string, size: number): Promise<void> {
  try {
    ...
    await fetch(`${base}/rest/v1/foyer_backups`, {
      ...
      body: JSON.stringify({ domain: host || "", scope: scope || "", size: size || 0 }),
...
export async function handleBackup(ctx: Ctx): Promise<Response | null> {
  ...
  const refIds = (str: string | null | undefined, kind: string): Set<number> => {
    ...
    let m;
    while ((m = re.exec(str))) set.add(parseInt(m[1]));
    return set;
  };
  if (route === "backup" && method === "GET") {
    ...
    if (usage.quota > 0 && usage.used >= usage.quota)
      return respond(
        {
          error: `Backup limit reached (${usage.used}/${usage.quota}). Ask the operator to raise your allotment.`,
          limit: usage.quota,
          used: usage.used,
        },
        403
    ...
    if (scope === "site") {
      ...
      out.data.collection_items = (items.results || []).map((it) => {
        ...
        return {
          collection_slug: idToSlug[it.collection_id],
          title: it.title,
          slug: it.slug,
      ...
      if (imgIds.size) {
        const ph = [...imgIds].map(() => "?").join(",");
        const r = await env.DB.prepare(
          `SELECT id, name, data, mime, size FROM images WHERE id IN (${ph})`
        )
          .bind(...imgIds)
          .all();
        out.data.images = r.results || [];
      } else out.data.images = [];
      if (fileIds.size) {
        const ph = [...fileIds].map(() => "?").join(",");
        const r = await env.DB.prepare(
          `SELECT id, name, data, mime, size FROM files WHERE id IN (${ph})`
        )
          .bind(...fileIds)
          .all();
        out.data.files = r.results || [];
      } else out.data.files = [];
    }
  ...
  if (route === "backup/restore" && method === "POST") {
    ...
    if (raw.length > 20 && _MAGIC.every((c, i) => raw[i] === c)) {
      ...
      if (!bkKey)
        return respond(
          { error: "this backup is encrypted, but no Foyer backup key is configured" },
          400
        );
      try {
        bundle = await openBinary(raw, bkKey);
      } catch (e) {
        return respond({ error: "could not decrypt — wrong key or corrupt file" }, 400);
      }
    } else {
      let txt = "";
      try {
      ...
      if (bundle && bundle.foyer_enc === 1) {
        ...
        try {
          bundle = await decryptBundle(bundle, bkKey);
        } catch (e) {
          return respond({ error: "could not decrypt — wrong key or corrupt file" }, 400);
        }
      }
    }
    if (!bundle || bundle.foyer_backup !== 1 || !bundle.data)
      return respond({ error: "not a valid .foyer backup" }, 400);
    await ensureAll();
    ...
    const insertAssets = async (
      ...
      if (toInsert.length) {
        const stmts = toInsert.map((a) =>
          env.DB.prepare(`INSERT INTO ${table} (name, data, mime, size) VALUES (?,?,?,?)`).bind(
            a.name || "",
            a.data,
            a.mime || defMime,
            a.size || 0
    ...
    const img = await insertAssets(data.images, "images", "image/jpeg");
    counts.images = img.added;
    const fil = await insertAssets(data.files, "files", "application/octet-stream");
    ...
    const remap = (s: string | null | undefined): string | null | undefined => {
      ...
      return String(s)
        .replace(/\/api\/images\/(\d+)/g, (m, id) =>
          imgMap[id] != null ? "/api/images/" + imgMap[id] : m
        )
        .replace(/\/api\/files\/(\d+)/g, (m, id) =>
          fileMap[id] != null ? "/api/files/" + fileMap[id] : m
        );
    };
      ...
      for (const p of data.pages) {
        ...
        stmts.push(
          env.DB.prepare(
            ...
            p.slug,
            pj,
            p.is_published != null ? (p.is_published ? 1 : 0) : 1,
            p.sort_order || 0
          )
        );
      }
      await env.DB.batch(stmts);
      counts.pages = stmts.length;
    }
      ...
      for (const it of data.collection_items) {
        ...
        stmts.push(
          env.DB.prepare(
            ...
            it.description || "",
            remap(it.content || ""),
            remap(it.cover_image || ""),
            it.sort_order || 0
          )
        );
      }
      if (stmts.length) await env.DB.batch(stmts);
      counts.collection_items = stmts.length;
    }
