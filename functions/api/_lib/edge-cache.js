...
export async function cacheLookup(ctx) {
  const { env: env, request: request, method: method } = ctx;
  if (method !== "GET" || !env.FOYER_KV || !CACHEABLE.test(ctx.route) || !isAnon(request))
    return null;
  const host = canonHost(env, request);
  let epoch;
  try {
    epoch = (await env.FOYER_KV.get("epoch:" + host, { cacheTtl: 60 })) || "0";
  } catch {
    return null;
  }
