...
export async function handleAuth(ctx) {
  ...
  async function captchaOk(token) {
    ...
    const rcSecret = env.RECAPTCHA_SECRET_KEY,
      tsSecret = env.TURNSTILE_SECRET_KEY;
  ...
  if (route === "account/sessions/revoke" && method === "POST") {
    const v = await currentVisitor();
    if (!v) return respond({ error: "unauthorized" }, 401);
    const { sid: sid } = await request.json().catch(() => ({}));
  ...
  if (route === "account/name" && method === "POST") {
    const v = await currentVisitor();
    if (!v) return respond({ error: "unauthorized" }, 401);
    const { name: name } = await request.json().catch(() => ({}));
  ...
  if (route === "auth/magic-link" && method === "POST") {
    ...
    const _hex = (r, g, b) =>
      ...
      [r, g, b]
        ...
        .join("");
    const dk = (t) => _hex(_a[0] * t, _a[1] * t, _a[2] * t);
    const lt = (t) =>
  ...
  if (route === "auth/magic-link/verify" && method === "POST") {
    const { token: token } = await request.json().catch(() => ({}));
  ...
  if (route === "auth/magic-link/status" && method === "POST") {
    const { request_id: request_id } = await request.json().catch(() => ({}));
  ...
  if (route === "auth/github" && method === "POST") {
    ...
    const { code: code } = await request.json().catch(() => ({}));
    if (!code) return respond({ error: "code required" }, 400);
  ...
  if (route === "auth/discord" && method === "POST") {
    ...
    const { code: code, redirect_uri: redirect_uri } = await request.json().catch(() => ({}));
    if (!code) return respond({ error: "code required" }, 400);
