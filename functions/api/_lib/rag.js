...
export async function ragUpsertPage(env, slug, text) {
  ...
  if (!chunks.length) {
    await piPost(env, "/rag/upsert", { db: ragDb(env), slug: slug, chunks: [] });
    return 0;
  }
  const vecs = await embed(env, chunks);
  const payload = chunks
    .map((t, i) => ({ text: t, embedding: vecs[i] || [] }))
    .filter((c) => c.embedding.length);
  await piPost(env, "/rag/upsert", { db: ragDb(env), slug: slug, chunks: payload });
  return payload.length;
}
export async function ragSearch(env, query, topk = 6) {
  const vecs = await embed(env, [query]);
  if (!vecs[0]) return [];
  const d = await piPost(env, "/rag/search", { db: ragDb(env), embedding: vecs[0], topk: topk });
  return (d && d.results) || [];
}
