...
const EMBED_MODEL = '@cf/baai/bge-base-en-v1.5';   // 768-dim, normalized
...
function piUrl(env, path) { return env.RAG_URL.replace(/\/$/, '') + path; }
function ragDb(env) { return env.RAG_DB || env.DB_HTTP_NAME || 'site'; }
...
async function piPost(env, path, body) {
  try {
    const r = await fetch(piUrl(env, path), { method: 'POST', headers: piHeaders(env), body: JSON.stringify(body) });
    if (!r.ok) return null;
    return await r.json();
  } catch { return null; }
}
...
export async function ragUpsertPage(env, slug, text) {
  ...
  if (!chunks.length) { await piPost(env, '/rag/upsert', { db: ragDb(env), slug, chunks: [] }); return 0; }
...
const _SKIP = new Set(['id', 'type', 'url', 'href', 'img', 'photo', 'src', 'bg_img', 'bg_image', 'image', 'avatar', 'cover_image', 'data', 'anchor', 'access_key', 'target', 'buy_url', 'btn_url', 'btn2_url', 'button_url']);
export function extractPageText(jsonStr) {
  let st;
  try { st = JSON.parse(jsonStr || '{}'); } catch { return ''; }
  const out = [];
  const collect = (o, d) => {
    ...
    if (typeof o === 'object') { for (const k in o) if (!_SKIP.has(k)) collect(o[k], d + 1); }
  };
