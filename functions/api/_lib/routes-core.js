...
export async function auditLog(env, actor, action, detail, request, db) {
  ...
  await db
    .prepare("INSERT INTO audit_log (actor, action, detail, path, ip) VALUES (?, ?, ?, ?, ?)")
    .bind(
...
async function aiEnabledForHost(env, host) {
  try {
    ...
    const r = await fetch(
      `${base}/rest/v1/foyer_sites?domain=eq.${encodeURIComponent(host)}&select=ai_enabled`,
...
export async function handleCore(ctx) {
  ...
  if (route === "config" && method === "GET") {
    ...
    return respond({
      ...
      magic_enabled: !!env.RESEND_API_KEY,
      turnstile_site_key: env.TURNSTILE_SITE_KEY || "",
      recaptcha_site_key: env.RECAPTCHA_SITE_KEY || "",
      plan: plan,
      is_pro: plan === "pro" || plan === "ultra",
      is_ultra: plan === "ultra",
    });
  }
  if (route && route.startsWith("sb/")) {
    ...
    if (route === "sb/announcements" && method === "GET") {
      const rows = await get(
        `foyer_announcements?scope=in.(global,${enc})&active=eq.true&select=id,message,level,hide_after,starts_at,ends_at&order=created_at.desc`,
        30
      );
      return respond(rows || []);
    }
    if (route === "sb/beat" && method === "POST") {
      ...
      await fetch(`${sbBase}/rest/v1/foyer_heartbeats?on_conflict=domain`, {
        ...
        body: JSON.stringify({
          domain: host,
          live_version: String(b.version || "").slice(0, 20),
    ...
    if (route === "sb/err" && method === "POST") {
      ...
      await fetch(`${sbBase}/rest/v1/foyer_errors`, {
        ...
        body: JSON.stringify({
          ...
          stack: String(b.stack || "").slice(0, 2e3),
  ...
  if (route === "ai/ask" && method === "POST") {
    ...
    if (!(await isPro(env, host)))
      return respond({ error: "“Ask this site” is a Pro feature." }, 403);
    {
      const rl = await rateLimit(env, `aiask:${host}:${clientIp(request)}`, 15, 60);
    ...
    if (!corpus) {
      ...
      for (const p of results || []) {
        let x = "";
        try {
          x = extractPageText(await decompressJson(p.page_json));
        } catch {}
    ...
    try {
      ...
      return respond({ error: "The assistant is busy — please try again." }, 502);
    }
    return respond({
      reply: ((out && out.response) || "").trim() || "Sorry — I’m not sure about that one.",
    });
  }
  ...
  if (route === "ai/reindex" && method === "POST") {
    if (!authed()) return respond({ error: "unauthorized" }, 401);
    if (!ragEnabled(env))
      return respond({ error: "Site search isn’t configured (needs the Pi RAG store)." }, 503);
    await env.DB.prepare(CREATE_PAGES).run();
    await env.DB.prepare("ALTER TABLE pages ADD COLUMN pw_hash TEXT")
      .run()
      .catch(() => {});
    ...
    for (const p of results || []) {
      let text = "";
      try {
        text = extractPageText(await decompressJson(p.page_json));
      } catch {}
      if (!text.trim()) continue;
      try {
        chunks += await ragUpsertPage(env, p.slug, `${p.title || ""}. ${text}`);
        pages++;
      } catch {}
    }
    return respond({ ok: true, pages: pages, chunks: chunks, index: await ragStats(env) });
  }
  ...
  if (route === "ai/page" && method === "POST") {
    ...
    if (!(await aiEnabledForHost(env, new URL(request.url).hostname)))
      return respond({ error: "The AI assistant is disabled for this site." }, 403);
    {
      const rl = await rateLimit(env, `aipage:${clientIp(request)}`, 30, 60);
      if (!rl.ok)
    ...
    if (site.name) {
      ...
      if (Array.isArray(site.pages) && site.pages.length) {
        siteCtx += ` Its pages (use slugs for links): ${site.pages
          .slice(0, 40)
          .map(
            (p) => `"${String(p.title || "").slice(0, 40)}" (${String(p.slug || "").slice(0, 40)})`
          )
          .join(", ")}.`;
      }
    ...
    const pageList = current.length
      ? current
          .map(
            (s, i) =>
              `[${i}] ${s.type}${s.heading ? ` — "${String(s.heading).slice(0, 40)}"` : s.name ? ` — "${String(s.name).slice(0, 40)}"` : ""}`
          )
          .join("\n")
      ...
      if (depth === 0 && j < txt.length) {
        try {
          ...
          if (
            o &&
            typeof o === "object" &&
            (typeof o.type === "string" || typeof o._op === "string")
          ) {
    ...
    return respond({ reply: reply, sections: sections });
  }
  ...
  if (route === "me" && method === "GET") {
    ...
    if (st)
      await env.DB.prepare("UPDATE sessions SET last_seen = datetime('now') WHERE token = ?")
        .bind(st)
        .run()
        .catch(() => {});
    return respond({ ok: true });
  ...
  if (route === "admin-check" && method === "GET") {
    const plan = await sitePlan(env, canonHost(env, request));
    return respond({ role: _adminRole || null, plan: plan });
  }
  return null;
}
