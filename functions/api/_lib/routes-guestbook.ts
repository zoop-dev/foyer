...
export async function handleGuestbook(ctx: Ctx): Promise<Response | null> {
  ...
  if (route === "guestbook" && method === "POST") {
    ...
    await ensureGB(env);
    const r = await env.DB.prepare("INSERT INTO guestbook (name, message, ip) VALUES (?,?,?)")
      .bind(name, message, clientIp(request))
      .run();
    return respond(
      {
  ...
  if (one && method === "DELETE") {
    ...
    await ensureGB(env);
    await env.DB.prepare("DELETE FROM guestbook WHERE id = ?")
      .bind(parseInt(one[1]))
      .run()
      .catch(() => {});
