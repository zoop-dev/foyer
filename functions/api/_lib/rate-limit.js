...
export async function rateLimit(env, key, limit, windowSec) {
  try {
    if (!_ready) { await env.DB.prepare(DDL).run().catch(() => {}); _ready = true; }
    ...
    await env.DB.prepare('INSERT INTO rate_limits (k, n, reset_at) VALUES (?, 1, ?) ON CONFLICT(k) DO UPDATE SET n = 1, reset_at = excluded.reset_at')
