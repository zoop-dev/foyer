...
export async function getModConfig(env, host) {
  ...
  if (hit && Date.now() - hit.at < 6e4) return hit.cfg;
  const { base: base, headers: H } = sb(env);
  let cfg = { ...DEFAULT_CFG };
  try {
    const r = await fetch(`${base}/rest/v1/foyer_meta?key=eq.moderation_config&select=value`, {
      headers: H,
    ...
    if (r.ok) {
      ...
      if (Array.isArray(rows) && rows[0] && rows[0].value)
        cfg = { ...DEFAULT_CFG, ...JSON.parse(rows[0].value) };
    }
  } catch (e) {}
  if (host) {
    try {
      const r = await fetch(
        `${base}/rest/v1/foyer_sites?domain=eq.${encodeURIComponent(host)}&select=moderation_config`,
        { headers: H, cf: { cacheTtl: 60, cacheEverything: true } }
      );
      if (r.ok) {
        const rows = await r.json();
        ...
        if (ov && typeof ov === "object") cfg = { ...cfg, ...ov };
      }
    } catch (e) {}
  }
  _cfgCache.set(k, { cfg: cfg, at: Date.now() });
  return cfg;
}
function inappropriate(u, cfg) {
  ...
  const domains =
    cfg.blocklist && cfg.blocklist.length ? BLOCK_DOMAINS.concat(cfg.blocklist) : BLOCK_DOMAINS;
  for (const d of domains) if (d && (host === d || host.endsWith("." + d))) return "blocked domain";
  const kws =
    cfg.keywords && cfg.keywords.length
      ? BLOCK_HOST_KEYWORDS.concat(cfg.keywords)
      : BLOCK_HOST_KEYWORDS;
  ...
  const attempt = async () => {
    try {
      const r = await fetch(url, {
        ...
        headers: { "User-Agent": "Mozilla/5.0 (compatible; FoyerLinkCheck/1.0)" },
      });
...
export async function moderatePage(jsonStr, cfg) {
  ...
  if (cfg.dead !== false || cfg.http !== false) {
    ...
    res.forEach((r, i) => {
      if (r.status === "fulfilled" && r.value) verdict[toCheck[i]] = r.value;
    });
  }
  const log = [];
  state.sections = state.sections.map((sec, idx) => {
    ...
    return {
      id: sec.id || Math.random().toString(36).slice(2, 9),
      type: "callout",
      variant: "warn",
      title: "",
      body: "this content was removed automatically.",
      icon: "@ban",
      pad: "sm",
    };
  });
  if (!log.length) return { json: jsonStr, log: [] };
  return { json: JSON.stringify(state), log: log };
}
...
export async function logModeration(env, host, slug, log) {
  ...
  await env.DB.batch(
    log.map((e) =>
      env.DB.prepare(
        "INSERT INTO moderation_log (slug, url, reason, block_type) VALUES (?,?,?,?)"
      ).bind(slug || "", e.url, e.reason, e.block)
    )
  );
  try {
    const { base: base, headers: H2 } = sb(env);
    ...
    await fetch(`${base}/rest/v1/foyer_moderation`, {
      method: "POST",
