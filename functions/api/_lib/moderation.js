...
const _cfgCache = /* @__PURE__ */ new Map();
...
async function checkDead(url, cfg) {
  const attempt = async () => {
    try {
      ...
      if (cfg.dead !== false && (r2.status === 404 || r2.status === 410)) return "dead link";
      if (cfg.http !== false && r2.status === 403) return "forbidden (403)";
      if (cfg.http !== false && r2.status === 401) return "unauthorized (401)";
      return "ok";
    } catch (e) {
