...
async function vapidAuthHeader(endpoint, publicB64url, privateD, subject) {
  ...
  const body = bytesToB64url(enc.encode(JSON.stringify({
    aud, exp: Math.floor(Date.now() / 1000) + 12 * 3600, sub: subject || 'mailto:admin@foyer.zo0p.dev',
  })));
...
async function encryptPayload(plaintext, uaPublicB64url, authB64url) {
  ...
  const ecdhBits = await crypto.subtle.deriveBits({ name: 'ECDH', public: uaKey }, eph.privateKey, 256);
...
export async function sendWebPush(subscription, payloadString, opts) {
  const body = await encryptPayload(payloadString, subscription.keys.p256dh, subscription.keys.auth);
  const auth = await vapidAuthHeader(subscription.endpoint, opts.vapidPublic, opts.vapidPrivate, opts.subject);
