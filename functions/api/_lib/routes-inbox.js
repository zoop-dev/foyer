...
const CREATE_INBOX = "CREATE TABLE IF NOT EXISTS inbox (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, email TEXT, subject TEXT, body TEXT, page TEXT, ip TEXT, created_at TEXT NOT NULL DEFAULT (datetime('now')), read_at TEXT)";
let _ready = false;
...
export async function handleInbox(ctx) {
  ...
  if (route === 'inbox' && method === 'POST') {
    ...
    else { try { const fd = await request.formData(); fd.forEach((v, k) => { data[k] = typeof v === 'string' ? v : ''; }); } catch (e) {} }
    if (data.botcheck) return respond({ ok: true });   // honeypot tripped — pretend success
    const known = new Set(['access_key', 'botcheck', 'subject', 'page', 'email', 'name']);
    const email = String(data.email || '').slice(0, 200).trim();
    const name = String(data.name || data.Name || '').slice(0, 120).trim();
    const subject = String(data.subject || 'New contact message').slice(0, 200).trim();

    const extra = Object.keys(data).filter((k) => !known.has(k) && String(data[k] || '').trim());
    let body = extra.map((k) => `${k}: ${String(data[k]).trim()}`).join('\n').slice(0, 5000);
    if (!body && data.message) body = String(data.message).slice(0, 5000);
    if (!email && !body) return respond({ error: 'Nothing to send.' }, 400);
    await ensureInbox(env);
    await env.DB.prepare('INSERT INTO inbox (name, email, subject, body, page, ip) VALUES (?,?,?,?,?,?)')
      .bind(name, email, subject, body, String(data.page || '').slice(0, 300), clientIp(request)).run();
    const note = notifyOwner(env, { title: '📥 ' + subject, body: (name || email || 'Someone') + ': ' + body.slice(0, 100), url: '/admin', icon: '/icons/favicon.svg' });
  ...
  if (route === 'inbox' && method === 'GET') {
    ...
    return respond({ messages: results || [], unread });
  }
  ...
  if (readM && method === 'POST') {
    if (!authed()) return respond({ error: 'unauthorized' }, 401);
    await ensureInbox(env);
    await env.DB.prepare("UPDATE inbox SET read_at = datetime('now') WHERE id = ?").bind(parseInt(readM[1])).run().catch(() => {});
    return respond({ ok: true });
  }
  if (one && method === 'DELETE') {
    if (!authed()) return respond({ error: 'unauthorized' }, 401);
    await ensureInbox(env);
    await env.DB.prepare('DELETE FROM inbox WHERE id = ?').bind(parseInt(one[1])).run().catch(() => {});
    return respond({ ok: true });
  }
  return null;
}
