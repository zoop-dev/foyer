...
async function siteState(env, host) {
  try {
    ...
    return { block: null, hideBranding: hideBranding };
  } catch {
...
const OG_BOT =
  /(facebookexternalhit|Facebot|Twitterbot|Slackbot|Discordbot|LinkedInBot|WhatsApp|TelegramBot|Pinterest|redditbot|Applebot|SkypeUriPreview|vkShare|embedly|Iframely|Google-InspectionTool)/i;
...
async function injectOg(ctx, url) {
  ...
  try {
    ...
    const setM = (id, val) => {
      if (!val) return;
      html = html.replace(new RegExp(`<meta\\b[^>]*\\bid="${id}"[^>]*>`), (tag) =>
        tag.replace(/content="[^"]*"/, `content="${escA(val)}"`)
      );
    };
    if (title) {
    ...
    html = html
      .replace(/<meta\b[^>]*\bid="og-url"[^>]*>/, (t) =>
        t.replace(/content="[^"]*"/, `content="${escA(pageUrl)}"`)
      )
      .replace(/<link\b[^>]*\bid="canonical"[^>]*>/, (t) =>
        t.replace(/href="[^"]*"/, `href="${escA(pageUrl)}"`)
      );
    if (ld) {
...
export async function onRequest(ctx) {
  const { request: request, next: next, env: env } = ctx;
  try {
    ...
    const { block: state, hideBranding: hideBranding } = await siteState(env, url.hostname);
    if (state) {
