export function preview(s, h) {
  ...
  return `<div style="${h.ff}${h.fc}padding:${p};">${headingHtml}<div style="border:1px dashed ${h.rgb(h.accent, 0.12)};padding:2rem;text-align:center;font-size:.62rem;letter-spacing:.22em;text-transform:uppercase;color:${h.rgb(h.accent, 0.32)};font-weight:100;">Tutorials — ${styleNote}${maxNote}</div></div>`;
}
export function render(s, h) {
  const p = s.pad === "sm" ? "1rem 2rem" : s.pad === "lg" ? "4rem 2rem" : "2rem 2rem";
  const uid = "tuts-" + Math.random().toString(36).slice(2, 8);
  return `<div id="${uid}"
            data-tuts="${h.A(s.tut_style || "cards")}"
            ...
            <p style="font-size:.62rem;letter-spacing:.18em;color:${h.rgb(h.accent, 0.3)};text-align:center;">Loading tutorials…</p>
          </div>`;
}
