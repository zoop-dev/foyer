import { foyerIconText } from "../blocks/render.js";
...
export function t(key) {
  return window.__i18n && window.__i18n[key] || window.__i18nAll && window.__i18nAll.en && window.__i18nAll.en[key] || key;
}
window.foyerT = t;
...
export const SESSION_KEY = "foyer_session";
export const SESSION_TTL = 7 * 24 * 60 * 60 * 1e3;
try {
...
export function showFallback(session) {
  dismissLoading();
...
export function pgRgb(hex, a) {
  const h = hex.replace("#", "");
  return `rgba(${parseInt(h.slice(0, 2), 16)},${parseInt(h.slice(2, 4), 16)},${parseInt(h.slice(4, 6), 16)},${a})`;
}
export function pgE(s) {
  return foyerIconText(
    String(s || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")
  );
}
marked.use({ breaks: true, gfm: true });
export const md = (s) => foyerIconText(marked.parse(String(s || "")).replace(/<\/a>(?=[A-Za-z0-9])/g, "</a> "));
