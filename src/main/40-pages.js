...
async function loadNav(session) {
  ...
  const pos = ["top", "bottom", "left", "right"].includes(navPref)
    ? navPref
    : ["top", "bottom", "left", "right"].includes(nav_position)
      ? nav_position
      : "top";
  const vertical = pos === "left" || pos === "right";
  const siteBg =
    getComputedStyle(document.documentElement).getPropertyValue("--site-bg").trim() || "#020a03";
  const siteAccent =
  ...
  const navLink = (p) =>
    `<a href="${hrefOf(p.slug)}" class="nav-a${hrefOf(p.slug) === cur ? " cur" : ""}">${p.title}</a>`;
  const pageLinks = pages
    .filter((p) => !(p.parent && slugSet.has(p.parent)))
    .map((p) => {
  ...
  const langPicker =
    _langs.length > 1
      ? `<span class="nav-lang-wrap" style="position:relative;display:inline-flex;align-items:center;">` +
        ...
        `<select class="nav-a nav-lang" aria-label="${foyerT("lang")}" style="appearance:none;-webkit-appearance:none;-moz-appearance:none;background:${siteBg}88;border:1px solid ${pgRgb(siteAccent, 0.22)};color:rgba(var(--site-muted-rgb),0.92);font-family:inherit;font-weight:300;font-size:.66rem;letter-spacing:.03em;padding:.34rem 1.5rem .34rem 1.65rem;border-radius:7px;cursor:pointer;outline:none;transition:border-color .2s;">` +
        _langs
          .map(
            (l) =>
              `<option value="${l}" style="color:#111;background:#fff;"${l === (window.foyerLang || _langs[0]) ? " selected" : ""}>${_lname(l)}</option>`
          )
          .join("") +
        `</select>` +
  ...
  if (_bell) {
    ...
    Promise.all([window.foyerPushConfig(), window.foyerPushState()]).then(([cfg, st]) =>
...
async function renderCollectionIndex(kind, session) {
  ...
  const accent =
    ...
    "#4dbd6a";
  const text =
    getComputedStyle(document.documentElement).getPropertyValue("--site-text").trim() || "#c8e6aa";
  const scene = document.getElementById("scene");
  scene.style.cssText =
    "position:fixed;inset:0;z-index:10;display:block;overflow-y:auto;padding:44px 0 0;";
  ...
  const cards = items
    .map(
      (t) =>
        `<a href="/${isRev ? "review" : "tutorials"}/${escAttr(t.slug)}" style="display:block;text-decoration:none;border:1px solid ${pgRgb(accent, 0.12)};background:${pgRgb(accent, 0.03)};transition:border-color .2s,transform .2s;">\n        ${t.cover_image ? `<img src="${escAttr(t.cover_image)}" style="width:100%;height:150px;object-fit:cover;display:block;" />` : `<div style="width:100%;height:150px;background:${pgRgb(accent, 0.06)};"></div>`}\n        <div style="padding:.9rem 1rem;">\n          ${isRev && t.rating ? `<div style="color:${accent};font-size:.82rem;letter-spacing:.1em;margin-bottom:.3rem;">${starStr(t.rating)}</div>` : ""}\n          <div style="font-weight:300;font-size:.95rem;letter-spacing:.03em;color:${pgRgb(text, 0.92)};">${pgE(t.title)}</div>\n          ${t.description ? `<div style="font-size:.72rem;font-weight:200;line-height:1.65;color:${pgRgb(text, 0.5)};margin-top:.35rem;">${pgE(t.description)}</div>` : ""}\n        </div>\n      </a>`
    )
    .join("");
...
function renderTextPage(state, session, pageTitle) {
  ...
  const cover = state.cover
    ? `<div style="margin:1.8rem 0 2.4rem;border:1px solid ${pgRgb(accent, 0.12)};border-radius:12px;overflow:hidden;"><img src="${escAttr(state.cover)}" alt="" style="width:100%;max-height:380px;object-fit:cover;display:block;" /></div>`
    : `<div style="height:1px;background:${pgRgb(accent, 0.12)};margin:1.8rem 0 2.4rem;"></div>`;
...
async function renderCollIndex(coll, session) {
  ...
  const card = (t) =>
    `<a href="${escAttr(base + "/" + t.slug)}" style="display:block;text-decoration:none;border:1px solid ${pgRgb(accent, 0.12)};background:${pgRgb(accent, 0.03)};border-radius:10px;overflow:hidden;transition:border-color .2s,transform .2s;">\n        ${t.cover_image ? `<img data-src="${escAttr(t.cover_image)}" alt="" decoding="async" style="width:100%;height:150px;object-fit:cover;display:block;background:${pgRgb(accent, 0.06)};" />` : `<div style="width:100%;height:150px;background:${pgRgb(accent, 0.06)};"></div>`}\n        <div style="padding:.9rem 1rem;"><div style="font-weight:300;font-size:.95rem;letter-spacing:.03em;color:${pgRgb(text, 0.92)};">${pgE(t.title || t.slug)}</div>${t.description ? `<div style="font-size:.72rem;font-weight:200;line-height:1.65;color:${pgRgb(text, 0.5)};margin-top:.35rem;">${pgE(t.description)}</div>` : ""}</div></a>`;
...
function renderCollItem(coll, item, session) {
  ...
  const cover = item.cover_image
    ? `<div style="margin:1.6rem 0 2.4rem;border:1px solid ${pgRgb(accent, 0.12)};border-radius:12px;overflow:hidden;"><img src="${escAttr(item.cover_image)}" alt="" style="width:100%;max-height:380px;object-fit:cover;display:block;" /></div>`
    : `<div style="height:1px;background:${pgRgb(accent, 0.12)};margin:1.8rem 0 2.4rem;"></div>`;
...
function renderReviewDetail(rev, session) {
  ...
  const stars = rating
    ? `<div style="font-size:1.1rem;letter-spacing:.15em;color:${accent};margin-bottom:1rem;">${"★".repeat(rating)}<span style="color:${pgRgb(text, 0.2)};">${"★".repeat(5 - rating)}</span></div>`
    : "";
  const cover = rev.cover_image
    ? `<img src="${escAttr(rev.cover_image)}" alt="" style="width:100%;max-height:340px;object-fit:cover;display:block;" />`
    : "";
...
function _cmtRow(c, accent, text) {
  const av = c.avatar
    ...
    : `<div style="width:34px;height:34px;border-radius:50%;flex:0 0 34px;display:flex;align-items:center;justify-content:center;background:${pgRgb(accent, 0.16)};color:${accent};font-size:.85rem;font-weight:300;">${pgE((c.name || "?").trim().charAt(0).toUpperCase())}</div>`;
...
async function foyerCommentsSection(target, session) {
  ...
  const text =
    getComputedStyle(document.documentElement).getPropertyValue("--site-text").trim() || "#c8e6aa";
  const loggedName =
    session && (session.name || session.email) ? session.name || session.email : "";
  const sec = document.createElement("section");
  sec.id = "foyerCmts";
  sec.style.cssText =
    "margin-top:3rem;padding-top:1.5rem;border-top:1px solid " + pgRgb(accent, 0.18) + ";";
  sec.innerHTML = `\n        <h3 style="font-weight:300;font-size:1.05rem;letter-spacing:.02em;color:${pgRgb(text, 0.9)};margin:0 0 1rem;">${pgE(foyerT("comments"))} <span id="cmtCount" style="color:${pgRgb(text, 0.4)};font-size:.85rem;"></span></h3>\n        <form id="cmtForm" style="margin-bottom:1.4rem;">\n          ${loggedName ? "" : `<input id="cmtName" maxlength="60" placeholder="${escAttr(foyerT("comment_name_ph"))}" style="width:100%;box-sizing:border-box;background:${pgRgb(accent, 0.06)};border:1px solid ${pgRgb(accent, 0.18)};border-radius:8px;padding:.6rem .8rem;color:${text};font-family:inherit;font-weight:200;font-size:.9rem;margin-bottom:.5rem;outline:none;" />`}\n          <textarea id="cmtBody" maxlength="2000" rows="3" placeholder="${escAttr(foyerT("comment_ph"))}" style="width:100%;box-sizing:border-box;background:${pgRgb(accent, 0.06)};border:1px solid ${pgRgb(accent, 0.18)};border-radius:8px;padding:.6rem .8rem;color:${text};font-family:inherit;font-weight:200;font-size:.9rem;resize:vertical;outline:none;"></textarea>\n          <div style="display:flex;align-items:center;gap:.8rem;margin-top:.5rem;">\n            <button type="submit" id="cmtPost" style="background:${accent};color:${pgRgb("#000", 0.85)};border:none;border-radius:8px;padding:.5rem 1.4rem;font-family:inherit;font-weight:300;font-size:.85rem;letter-spacing:.03em;cursor:pointer;">${pgE(foyerT("comment_post"))}</button>\n            <span id="cmtErr" style="font-size:.75rem;font-weight:200;color:#e88;"></span>\n          </div>\n        </form>\n        <div id="cmtList" style="font-family:'Josefin Sans',sans-serif;"></div>`;
  ...
  const paint = (rows) => {
    ...
    listEl.innerHTML = rows.length
      ? rows.map((c) => _cmtRow(c, accent, text)).join("")
      : `<p style="font-weight:200;font-size:.85rem;color:${pgRgb(text, 0.45)};padding:.6rem 0;">${pgE(foyerT("comment_none"))}</p>`;
  };
  let rows = [];
  try {
    rows = await fetch("/api/comments?target=" + encodeURIComponent(target)).then((r) =>
      r.ok ? r.json() : []
    );
  } catch {}
  ...
  sec.querySelector("#cmtForm").addEventListener("submit", async (e) => {
    ...
    if (!name || !body) {
      errEl.textContent = foyerT("comment_name_ph");
      return;
    }
    const btn = sec.querySelector("#cmtPost");
    const orig = btn.textContent;
    btn.disabled = true;
    btn.textContent = foyerT("comment_sending");
    try {
...
function fetchPage(slug, session) {
  ...
  _pageCache.set(slug, { t: Date.now(), p: p });
  return p;
}
...
function foyerOpenSearch() {
  ...
  const q = ov.querySelector("#fsq"),
    res = ov.querySelector("#fsr");
  ...
  const run = async () => {
    ...
    res.innerHTML = hits.length
      ? hits
          .map(
            (h) =>
              `<a href="${escAttr(hrefOf(h.item.s))}" data-fs style="display:block;padding:.75rem 1.3rem;border-bottom:1px solid ${pgRgb(accent, 0.08)};text-decoration:none;color:${text};"><div style="font-weight:300;font-size:.95rem;">${pgE(h.item.t || h.item.s)}</div><div style="font-size:.74rem;font-weight:200;color:${pgRgb(text, 0.5)};margin-top:.2rem;line-height:1.5;">${_foyerSnippet(h.item.x, term)}</div></a>`
          )
          .join("")
      : `<div style="padding:1.1rem 1.3rem;color:${pgRgb(text, 0.5)};font-weight:200;font-size:.88rem;">No results for &ldquo;${pgE(term)}&rdquo;.</div>`;
  };
...
function mountAskWidget(settings, session) {
  ...
  const corner = ["br", "bl", "tr", "tl"].includes(settings.ask_corner)
    ? settings.ask_corner
    : "br";
  const cs = getComputedStyle(document.documentElement);
  const accent = cs.getPropertyValue("--site-accent").trim() || "#4dbd6a";
  ...
  const circle =
    settings.ask_color && /^#?[0-9a-fA-F]{3,8}$/.test(settings.ask_color.replace(/^#/, ""))
      ? settings.ask_color[0] === "#"
        ? settings.ask_color
        : "#" + settings.ask_color
      : accent;
  ...
  const toggle = (open) => {
    ...
    if (open) {
      if (!greeted) {
        greeted = true;
        addMsg("assistant", foyerT("ask_hi"));
      }
  ...
  root.querySelector("#askForm").addEventListener("submit", async (e) => {
    ...
    try {
      ...
      typing.remove();
      const reply = r.ok
        ? d.reply || "Sorry — I’m not sure about that."
...
function renderLockedPage(slug, page, session, bad) {
  ...
  scene.querySelector("#pwform").addEventListener("submit", async (e) => {
    ...
    let data = null;
    try {
      const r = await fetch(`/api/pages?slug=${encodeURIComponent(slug)}`, { headers: headers });
      data = await r.json();
    } catch (err) {}
    if (!data || data.locked) {
...
const _isTrText = (s) =>
  typeof s === "string" &&
  s.trim().length > 1 &&
  !/^(yes|no|true|false|on|off)$/i.test(s.trim()) &&
...
function _trOverlay(show, done, total) {
  ...
  const cs = getComputedStyle(document.documentElement);
  const bg = cs.getPropertyValue("--site-bg").trim() || "#020a03",
    accent = cs.getPropertyValue("--site-accent").trim() || "#4dbd6a",
    text = cs.getPropertyValue("--site-text").trim() || "#c8e6aa";
  if (!el) {
  ...
  if (lbl)
    lbl.textContent =
      (foyerT("translating") || "Translating") + "…" + (total > 1 ? ` ${done}/${total}` : "");
}
...
async function loadAndShow(session) {
  ...
  try {
    _wantsAdmin =
      sessionStorage.getItem("foyer_admin_return") === "1" ||
      new URLSearchParams(location.search).get("admin_return") === "1";
  } catch {}
...
window.foyerLoadAndShow = loadAndShow;
window.foyerLoadNav = loadNav;
