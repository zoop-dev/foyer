    ...
    function foyerSignIn() {
      const err = document.getElementById('gate-err');
      if (!window.Foyer) { if (err) err.textContent = 'Foyer is still loading — try again in a moment.'; return; }
      window.Foyer.signIn({ exchangeUrl: '/api/auth/foyer' }).then(_foyerSession).catch((e) => {
        if (e && e.error === 'popup_blocked' && err) err.textContent = 'Please allow popups to sign in with Foyer.';
      });
    }
