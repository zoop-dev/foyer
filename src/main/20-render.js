      ...
      switch (s.type) {
        case 'collection': {
          ...
          const cards = items.map(it=>{
            ...
            return `<div class="coll-card" style="border:1px solid ${pgRgb(accent,.12)};background:${pgRgb(accent,.03)};position:relative;">${badge}${coverHtml}${thumbs}<div style="padding:.7rem .9rem;"><div style="font-weight:300;font-size:.88rem;color:${pgRgb(text,.9)};">${pgE(it.name||'')}</div>${it.note?`<div style="font-size:.7rem;font-weight:200;line-height:1.6;color:${pgRgb(text,.5)};margin-top:.25rem;">${pgE(it.note)}</div>`:''}${(it.buy==='yes'&&it.buy_url)?`<a href="${escAttr(it.buy_url)}" target="_blank" rel="noopener" style="display:inline-block;margin-top:.6rem;background:${accent};color:${bg};font-size:.64rem;font-weight:400;letter-spacing:.08em;text-transform:uppercase;padding:.45rem .85rem;border-radius:6px;text-decoration:none;">${pgE(it.buy_label||'Buy')}</a>`:''}</div></div>`;
          }).join('');
    ...
    function initCountdowns(root) {
      root.querySelectorAll('[data-foyer-cd]').forEach(el => {
        ...
        const tick = () => {
          ...
          if (diff <= 0) { el.innerHTML = `<div style="font-size:1.2rem;font-weight:300;">${el.dataset.done || ''}</div>`; if (iv) clearInterval(iv); return; }
          diff = Math.floor(diff / 1000);
          const v = [Math.floor(diff / 86400), Math.floor(diff % 86400 / 3600), Math.floor(diff % 3600 / 60), diff % 60];
          slots.forEach((sl, i) => { sl.textContent = String(v[i]).padStart(2, '0'); });
        };
        tick();
    ...
    function initForms(root) {
      const showErr = (f, msg) => {
        ...
        if (!e) { e = document.createElement('div'); e.className = 'w3-err'; e.style.cssText = 'font-size:.75rem;color:#e0608a;margin-top:.5rem;'; f.appendChild(e); }
        e.textContent = msg;
      };
        ...
        f.addEventListener('submit', e => {
          ...
          fetch('https://api.web3forms.com/submit', { method: 'POST', body: new FormData(f) })
            .then(r => r.json()).then(d => {
              if (d && d.success) { f.innerHTML = `<p style="font-size:.92rem;font-weight:300;padding:.8rem 0;">${f.dataset.done || 'Thanks!'}</p>`; }
              else { if (btn) { btn.disabled = false; btn.textContent = orig || 'Try again'; } showErr(f, (d && d.message) || 'Could not send.'); }
            })
      ...
      root.querySelectorAll('[data-coll-embed]:not([data-ce])').forEach((el) => {
        ...
        const load = async (attempt) => {
          ...
          try {
            ...
            if (ctrl) timer = setTimeout(() => ctrl.abort(), 7000);
            const r = await fetch('/api/collections/' + encodeURIComponent(cs) + '/items', { headers: sessionHeaders(session), signal: ctrl ? ctrl.signal : undefined, cache: 'no-store' });
