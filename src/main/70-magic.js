function promptName(session, done) {
  ...
  el.id = "ml-name-modal";
  el.style.cssText =
    "position:fixed;inset:0;z-index:100002;display:flex;align-items:center;justify-content:center;padding:1.5rem;background:rgba(2,8,3,.78);backdrop-filter:blur(6px);";
  ...
  el.querySelector("#mlNameForm").addEventListener("submit", async (e) => {
    ...
    await fetch("/api/account/name", {
      method: "POST",
      headers: { "Content-Type": "application/json", ...window.foyerSessionHeaders(session) },
      body: JSON.stringify({ name: name }),
    }).catch(() => {});
    session.name = name;
    window.foyerSetSession(session);
...
function magicContinue(session) {
  window.foyerDismissGate();
  if (session.needs_name) promptName(session, () => window.foyerLoadAndShow(session));
  else window.foyerLoadAndShow(session);
}
function showMagicApprovedScreen(session) {
  ...
  el.id = "ml-approved";
  el.style.cssText =
    'position:fixed;inset:0;z-index:100001;background:#070b07;display:flex;align-items:center;justify-content:center;font-family:"Josefin Sans",sans-serif;padding:2rem;';
...
function openMagicModal() {
  ...
  const requestLink = (email) =>
    fetch("/api/auth/magic-link", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: email, captcha_token: window.foyerGetCaptchaToken() }),
    })
      .then((r) => r.json())
      .catch(() => null);
  const open = () => {
            ...
            : data?.error === "vpn_blocked"
              ...
              : data?.error === "signup_limited"
                ...
                : data?.error === "captcha_failed"
                  ? "Bot check failed — please complete the captcha and try again."
