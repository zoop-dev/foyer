    ...
    function startMagicBtn() {
      ...
      const stopCooldown = () => { if (cooldownTimer) { clearInterval(cooldownTimer); cooldownTimer = null; } };
      ...
      function startCooldown() {
        ...
        cooldownTimer = setInterval(() => {
          n--;
          if (n <= 0) { stopCooldown(); resendLink.disabled = false; resendLink.textContent = 'Resend link'; }
          else resendLink.textContent = `Resend in ${n}s`;
        }, 1000);
      }

      function showWaiting() {
        spinner.style.display = ''; sentIcon.style.display = 'none';
      ...
      const close = () => {
        ...
        setTimeout(() => {  // reset for next time
          formView.style.display = ''; sentView.style.display = 'none';
          send.disabled = false; send.textContent = 'Send sign-in link'; err.textContent = '';
          spinner.style.display = ''; sentIcon.style.display = 'none';
          sentMsg.innerHTML = sentMsgDefault; resendErr.textContent = '';
          resendLink.disabled = true; resendLink.textContent = `Resend in ${COOLDOWN}s`;
        }, 250);
      };
      ...
      resend.addEventListener('click', () => {
        stopPoll(); stopCooldown();
        sentView.style.display = 'none'; formView.style.display = '';
        send.disabled = false; send.textContent = 'Send sign-in link';
        spinner.style.display = ''; sentIcon.style.display = 'none';
        sentMsg.innerHTML = sentMsgDefault; resendErr.textContent = ''; err.textContent = '';
        input.focus();
      });

      resendLink.addEventListener('click', async () => {
        if (resendLink.disabled || !currentEmail) return;
        resendLink.disabled = true; resendLink.textContent = 'Sending…';
        showWaiting();
      ...
      const startPoll = (requestId) => {
        ...
        pollTimer = setInterval(async () => {
          ...
          if (s.status === 'approved') {
            ...
            stopCooldown(); resendLink.disabled = false; resendLink.textContent = 'Resend link';
          }
        }, 2500);
      };
