...
window.foyerSessionHeaders = sessionHeaders;
...
function showUpdateOverlay(pendingUiVersion, newVer) {
  ...
  const cacheNewVersion = () => {
    if (pendingUiVersion) localStorage.setItem(UI_VERSION_KEY, pendingUiVersion);
  };
  ...
  const _fIcon = (w, h, style) =>
    `<svg viewBox="0 0 44 50" width="${w}" height="${h}" fill="none" stroke="rgba(var(--site-accent-rgb),.85)" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;${style || ""}"><path d="M5 46 V24 a16 16 0 0 1 32 0 V46"/><path d="M15 46 V28 a6 6 0 0 1 12 0 V46"/></svg>`;
  ...
  setTimeout(() => {
    ...
    el.id = "_ver_overlay";
    el.style.cssText =
      'position:fixed;inset:0;z-index:9997;background:rgba(var(--site-bg-rgb),0);display:flex;align-items:center;justify-content:center;font-family:"Josefin Sans",sans-serif;transition:background .4s ease;';
...
function showBannedScreen() {
  ...
  el.id = "banned-screen";
  el.style.cssText =
    'position:fixed;inset:0;z-index:9997;background:var(--site-bg);display:flex;align-items:center;justify-content:center;flex-direction:column;gap:1.2rem;font-family:"Josefin Sans",sans-serif;';
