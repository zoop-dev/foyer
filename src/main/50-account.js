import { SESSION_KEY, escAttr, escHtml, getSession, hookHover } from "./10-core.js";
import { loadNav } from "./40-pages.js";
...
function relTime(ts) {
  if (!ts) return "";
  const t = /* @__PURE__ */ (/* @__PURE__ */ new Date(String(ts).replace(" ", "T") + "Z")).getTime();
  if (isNaN(t)) return "";
...
async function loadAccountSessions() {
  ...
  wrap.innerHTML = data.sessions.map(
    (s) => `
  ...
  wrap.querySelectorAll(".acct-revoke").forEach((b) => {
    ...
    b.addEventListener("click", async () => {
      ...
      await fetch("/api/account/sessions/revoke", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...sessionHeaders(getSession()) },
...
async function openAccountPanel() {
  ...
  if (acc && !acc.error) {
    ...
    if (acc.role === "owner" || acc.role === "admin")
      tags.push(`<span class="acct-tag acct-tag-role">${escHtml(acc.role)}</span>`);
    document.getElementById("acctTags").innerHTML = tags.join("");
    document.getElementById("acctMeta").textContent = `Member since ${relTime(acc.first_seen)} · ${acc.visit_count} visit${acc.visit_count === 1 ? "" : "s"}`;
